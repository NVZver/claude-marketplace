...
- **`output`** — Apply to every human-facing output. Enforces 5 format golden rules (structured, minimal, formatted, sourced, concrete) — see [`core/CLAUDE.md`](./CLAUDE.md). Each component picks its own format within these rules. Since v0.5.3, *sourced* also requires a `[plugin:skill]` marker on the first line of every substantive response (e.g., `[core:output]`, `[lsa:lsa-specify]`).
