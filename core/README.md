...
- **`output`** — Apply to every human-facing output. Enforces 5 format golden rules (structured, minimal, formatted, sourced, concrete) — see [`core/CLAUDE.md`](./CLAUDE.md). Each component picks its own format within these rules. Since v0.5.4, *sourced* requires a one-line trace directive at the top of every marketplace instructional file; on load the agent prints `=============== [<file>] [<plugin>] ===============` verbatim, one line per loaded file, in load order, before the response body. Replaces the v0.5.3 single-line `[plugin:skill]` marker.
