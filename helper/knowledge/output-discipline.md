...
- **Closing picker.** Close with `AskUserQuestion` when a real next step exists; otherwise close with no picker (see § *Genuine fork — operating definition* below). End cleanly — no filler, no `"Anything else?"`. Skip the closing picker entirely on `Skill()` handoff. Per `vision/specs/features/2026-05-22-helper-assistant-refactor/requirements.md` F2 / AC4 and `vision/specs/features/2026-05-22-askuserquestion-audit/tasks.md` C3.
