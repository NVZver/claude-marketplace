...
- **Closing picker.** Close with `AskUserQuestion` only when a genuine fork remains after the answer (see § *Genuine fork — operating definition* below). Otherwise end cleanly — no filler, no `"Anything else?"`. Skip the closing picker entirely on `Skill()` handoff. Per `.lsa/features/2026-05-22-helper-assistant-refactor/requirements.md` F2 / AC4.
