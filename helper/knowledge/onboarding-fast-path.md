...
| 2 | Get started with LSA | "how do I get started with LSA", "where do I start with LSA", "first steps LSA" | [`README.md#install`](../../README.md#install) + [`README.md#lsa`](../../README.md#lsa) | Install the plugins, then run `/lsa:init` or `/lsa:new` — the four-command build cycle follows. |
...
| 5 | What is `lsa` | "what is LSA", "what does lsa do", "what is Living Spec Architecture" | [`README.md#lsa`](../../README.md#lsa) | Definition + the four-command build cycle with illustrative output. |
...
| 7 | What is `management` | "what is management", "what does management do", "what is `start-feature`" | [`README.md#management`](../../README.md#management) | Pitch-shaping user flow + the human approval gate before `management:roadmap` decomposes the pitch into LSA-ready epics. |
...
Catalog size v2: **8 rows**. Floor named in `requirements.md` NF2 (install / start / what-is + {core, lsa, helper} / how-do-I-run); v2 extends the *what-is* row set to cover all five shipped plugins ({core, lsa, helper, management, prompt-engineer}) per [`.lsa/pitches/readme-and-knowledge-base.md`](../../.lsa/pitches/readme-and-knowledge-base.md) "Solution sketch". Expansion happens here, not in [`../agents/helper.md`](../agents/helper.md) (F6 — catalog is data, not code). The canonical heading-name registry is [`knowledge/index.md`](../../knowledge/index.md) — any heading rename in `README.md` must update both files in the same commit.
...
- **Canonical subjects only.** Catalog subjects are `marketplace`, `core`, `lsa`, `helper`, `management`, `prompt-engineer`. Other plugin names (`dev-plugin`, `atlassian`, `supabase`, `frontend-design`) are NOT onboarding subjects — they live in scope 2 of [`./knowledge-scope.md`](./knowledge-scope.md) and require a deep-read.
