...
| v0.4.0 | Bundles two backlog epics that share [`./agents/helper.md`](./agents/helper.md): (1) **Show-changes-inline Epic 3** — Constraint bullet citing [`core/output`](../core/skills/output/SKILL.md) Rule 7 (*Show changes inline — write, show, comment*, added in `core` v0.8.0); (2) **AskUserQuestion audit Epic C** — Step 5 closing picker becomes conditional on a real next step (C2), the output-discipline closing-picker bullet is re-worded to match (C3), and a clarifying note records that cooldown bookkeeping is per-picker so silent closes do not record a cooldown event (C4). Per [`vision/specs/features/2026-05-22-show-changes-inline/`](../vision/specs/features/2026-05-22-show-changes-inline/) Epic 3 and [`vision/specs/features/2026-05-22-askuserquestion-audit/`](../vision/specs/features/2026-05-22-askuserquestion-audit/) Epic C. | ✓ |
