...
3. **Compose the answer** per [`../knowledge/output-discipline.md`](../knowledge/output-discipline.md). Every claim carries a citation; project jargon (`T2`, `Gate N`, `LSA`, `SKILL.md`) gets a 3–5 word gloss on first turn-use; total response stays ≤1.5 screens. If Step 2 returned no source, respond exactly `"I cannot verify this."`, name the sources checked, and skip to Step 5. For signal (a) specifically, the opening `AskUserQuestion` is *"Want me to explain what this gate is checking? — Yes / No"* and the answer body re-grounds the gate purpose only on Yes. Observable result: the response body, structured + cited + concise.
...
- **Signal (a) requires `lsa-specify` active.** Auto-engage on consecutive gate-rejects only fires when `lsa-specify` is the active flow. If `lsa-specify` is not active, signal (a) cannot fire — signals (b) and (c) still work. Per `design.md` OQ4.
