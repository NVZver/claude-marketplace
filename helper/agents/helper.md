...
- **No subagent spawn.** Tools list deliberately omits the `Agent` tool. Helper uses `Read` / `Grep` / `Glob` directly. If implementation reveals this is too narrow, re-enter `lsa:discover` for a spec amendment — do not silently widen. Per `vision/specs/features/2026-05-21-helper-agent/design.md` OQ3 resolution.
