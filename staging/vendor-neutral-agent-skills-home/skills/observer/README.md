...
```
npx skills add <org>/<repo> --skill core/ground-rules --skill core/output --skill observer/observe --skill observer/verify-checkpoint
```

Install `core`'s `ground-rules` + `output` too — `observer` cites them for fact-grounding and format discipline. See the root [`CONTRIBUTING.md`](../../CONTRIBUTING.md) for the manual-copy fallback.
...
observer:observe          → confirm role (kickoff) → [interviewer: scaffold red exercise]
                          → re-invoke each cycle → per-cycle feedback or silence (active role's bundle)
...
observer:verify-checkpoint → grade ONE signalled increment (the core unit), via either:
                          (a) per-increment dispatch by lsa:delegate, OR (b) standalone re-invoke-on-change rider
