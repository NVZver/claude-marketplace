...
Knowledge, not Actor. This file states *what each role is* — its lens, voice, and cadence (and, for interviewer, its difficulty rules) — as data. It prescribes no steps and owns no Goal/Input/Steps/Output/Constraints; the [`../observe/SKILL.md`](../observe/SKILL.md) Actor reads the active role's bundle here and applies it generically. The role set is **rubber-duck, pair-programmer, interviewer, custom**.
