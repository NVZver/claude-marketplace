...
- The example's file-load trace is now explicitly attributed to the first *emitting* cycle, citing `skills/core/output/SKILL.md` Rule 4 *Silent-cycle exemption* (new in `core` 0.20.0) — the trace is owed on first emission, not on the loads that preceded it.
...
- **`observer:verify-checkpoint` Actor skill** ([`skills/verify-checkpoint/SKILL.md`](verify-checkpoint/SKILL.md))
