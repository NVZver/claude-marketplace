...
Every fast-path answer carries the source inline: a verbatim quote of the answering line(s) plus a `file:line` (or `file#heading-anchor`) citation the reader can locate in seconds. This inherits [`../output/SKILL.md`](../output/SKILL.md) Rule 4 (Sourced) and Rule 7 (Show changes inline — quote before commentary), and [`../ground-rules/SKILL.md`](../ground-rules/SKILL.md) Rule 1 (every factual claim carries a source + searchable quote). A fast-path answer without a verbatim quote-back fails the pattern.

## Callers

| Caller | Source-of-truth file | Trigger shape |
|---|---|---|
| [`../../manager/next/SKILL.md`](../../manager/next/SKILL.md) | `${specs_root}/roadmap.yaml` `items:` (via `scripts/roadmap-row.sh`) | "what's next" (reserves full dispatch for "recommend an order") |
