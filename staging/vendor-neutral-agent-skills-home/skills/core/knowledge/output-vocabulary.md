...
Verdict labels for human-facing outputs. Pure constants — Knowledge, not Actor. Components whose chosen format uses a verdict line (e.g., `verify` reports, `flow-selector` confirms) pick from this table; components without a verdict moment don't render one.

Cited by [`core/output`](../output/SKILL.md). For when to render a verdict, see the output skill body.
