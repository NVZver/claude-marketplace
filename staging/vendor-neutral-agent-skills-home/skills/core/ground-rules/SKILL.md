...
> **Trace.** On load, print first: `=============== [skills/core/ground-rules/SKILL.md] [core] ===============`
...
Per `.lsa/constitution.md` §2 principle 7 — *"The human owns intent; the system absorbs reality."*
...
Ask the human with labelled options and one-line outcomes (per `.lsa/constitution.md` §2 principle 9 + `core/output`); the host maps this to its native picker where one exists, else renders text-formatted options.
...
Do not guess when you can check. If you genuinely cannot check, say so plainly rather than filling the gap with a confident guess. In Claude Code, prefer the substrate's native file primitives (`Read` / `Edit` / `Write`) over shell `cat` / `sed` / `echo` — per `.lsa/constitution.md` §2 principle 9.
