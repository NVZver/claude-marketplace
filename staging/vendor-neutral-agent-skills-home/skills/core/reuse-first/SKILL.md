...
The reuse ladder catches reinvention and over-delivery *before* the diff exists, not after — the authoring-time reflex between the spec ("what") and [`lsa:reconcile`'s after-the-fact "only" check](../../lsa/reconcile/SKILL.md). On prose/analysis tasks (no code to author) this skill stays silent.
...
- Deliver only the minimum the task requires, per [`ground-rules` Rule 4 *Deliver only what was asked*](../ground-rules/SKILL.md); the after-the-fact scope check is [`lsa:reconcile`'s "only" check](../../lsa/reconcile/SKILL.md) — this skill is the before-the-diff reflex, not a restatement of either.
