...
[`../knowledge/output-vocabulary.md`](../knowledge/output-vocabulary.md) §"Verdicts"
...
This rule generalizes the 8-element drift block originally pioneered by [`reconcile`](../../lsa/reconcile/SKILL.md), since absorbed here — the *Single-change template* below is the canonical form.
...
For **approval-gated artifacts** — anything whose existence depends on a pending human gate (pitches, specs, roadmap rows, generated prompt files) — the order inverts: **show → approve → write**. Deliver the full content first (Delivery test below), run the gate, write the file only on approve. Nothing lands on disk "as a draft" before its gate. On reject, nothing is written. Prior art: [`lsa:init`](../../lsa/init/SKILL.md) Step 3 (*"show, approve, write"*) and [`lsa:revise-constitution`](../../lsa/revise-constitution/SKILL.md) Steps 3–4.
...
- **inside a decision gate** (question text, option descriptions, or option `preview`) — the host's native picker where one exists, else text-rendered options.
...
A correctly-prompted skill can still mis-execute at runtime — no automated PR-time check exists for that today; the human reviewing the turn is the runtime backstop.

---

Substrate selection — see `.lsa/constitution.md` §2 principle 9 (*"Substrate-native first"*).

Verdict labels — see [`../knowledge/output-vocabulary.md`](../knowledge/output-vocabulary.md).
