...
The roll-up invents **no new change-table format**. Its files section reuses the [`core/output`](../../core/output/SKILL.md) Rule 7 **compressed inspection table** verbatim (`| # | file:line | type | summary |`), and groups files by **Conventional Commits `type(scope)`** — both already exist. This is the "one report contract" the pitch's rabbit-hole 7 requires.
