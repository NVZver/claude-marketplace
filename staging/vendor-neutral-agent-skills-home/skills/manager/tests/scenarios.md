...
- **SETUP:** Two epics: `manager-consistency/thresholds` (write-set:
  `skills/manager/implement/SKILL.md`, `skills/manager/knowledge/parallel-dispatch.md`) and
  `manager-consistency/rollup-tighten` (write-set: `skills/manager/knowledge/fleet-rollup.md`,
  `skills/manager/implement/SKILL.md`). No `--parallel` flag given.
- **PROBE:** User message: "These two are basically independent — one's about
  ...
  `skills/manager/implement/SKILL.md` (file overlap, §1.1).
