...
2. **Load pitch outlines, not pitch bodies.** Run `bash scripts/pitch-query.sh outline <slug>...` for the candidate slugs — it prints title + first line of `## Problem` + first line of `## Appetite` per pitch with a `path:line` citation, capped and deterministic (zero model tokens). That is what sequencing needs; a full body is not. **Do not read a full pitch file during sequencing.** Read `${specs_root}/pitches/<slug>.md` in full only *after* the user picks an item at Step 5, or when an outline is genuinely insufficient to rank a candidate — in which case name the candidate and say why the outline was not enough, so the over-read is observable rather than habitual. An outline the script marks `INCOMPLETE` (the pitch has no `## Problem` / `## Appetite` section) is the one sanctioned trigger for reading that body during sequencing — the marker *is* the "outline was not enough" justification, already named by the script. Slugs the script reports as having no pitch are flagged for Mode 1b (Tidy) — that is script class 1 (missing-pitch). Only if the script exits non-zero fall through to model-side reads. Observable result: one capped outline per candidate (measured 25× less than the equivalent full reads on a 5-candidate pass — `scripts/tests/no-wholefile-pitch-read.sh:10`), plus any named-and-justified full read.
...
7. **Report hygiene findings — report-only, no write path.** Return each finding in the payload as an observation: the row quoted with `file:line` plus what contradicts it. **Do not propose a gated row diff and do not write the roadmap from this mode.** Applying hygiene fixes belongs to the `manager:check` skill, which runs inline, owns the gates, and writes the approved rows itself (manager 0.20.0). The dispatchers that reach this mode — `manager:next` (recommend-only, never writes the roadmap) and `manager:decompose` (gates the epic list only) — run no hygiene gate, so a row diff returned here would have no gate to reach and no writer to apply it. Surface the findings as context for the recommendation and point the user at `manager:check` to act on them. Observable result: hygiene findings returned as cited observations, with `manager:check` named as the way to apply them; nothing written from this mode.
...
- **Inherits `core/ground-rules`** -- per [`../../core/ground-rules/SKILL.md`](../../core/ground-rules/SKILL.md).
- **Inherits `core/output`** -- per [`../../core/output/SKILL.md`](../../core/output/SKILL.md).
- **Gates belong to the dispatcher.** A decision gate and the `Skill` tool are unavailable in subagent context; never attempt them, never fake a gate result. Return pending gates and the staged `lsa:discover` seed in the payload; the dispatching skill (`manager:next` / `manager:decompose`) runs the gates and invokes the handoff. If invoked directly (not as a subagent) the agent may interact with the user, but still follows the same propose-then-return contract.
...
- **Show changes inline.** Every roadmap write is echoed back inline before commentary -- write, show, comment. Quote the new/changed row with `file:line`; never *"roadmap updated"* or *"go check the roadmap"* without the row. Per [`../../core/output/SKILL.md`](../../core/output/SKILL.md) Rule 7.
- **No persona theater.** No name, no greeting. "Project-manager" is a role descriptor, not a character.
- **Ownership over automation** -- per [`../../../.lsa/constitution.md`](../../../.lsa/constitution.md) section 0: *"the system does not think for the human; it makes the human think."* The agent recommends; the human decides. All roadmap writes require explicit approval.
