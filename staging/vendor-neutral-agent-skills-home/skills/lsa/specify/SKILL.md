...
4. Show the full draft spec (per [`core/output`](../../core/output/SKILL.md) Rule 7 *Delivery test*); take human approval; **then** write `requirements.md` + the `<flow>.feature` files — show → approve → write per Rule 7 *Authorization boundary*. (→ approved spec on disk)
