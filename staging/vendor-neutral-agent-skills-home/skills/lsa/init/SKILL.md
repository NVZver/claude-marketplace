...
1. Resolve config — `constitution` / `specs_root` / `mode` (defaults: `.lsa/constitution.md`, `.lsa/`, `code`); print it. (→ config)
2. **Greenfield:** create under `${specs_root}/` — `main.spec.md`, **`roadmap.yaml`** (YAML ledger SoT — starter below), `research-backlog.md`, `modules/`, `features/`, `standards/{code,testing}.md`, `archive/`; fill `standards/` from the constitution. **Do not create `roadmap.md`.** Starter ledger (write exactly):
   ...
   After writing, quote the new `roadmap.yaml` inline (and a one-line list of the other created paths) per [`../../core/output/SKILL.md`](../../core/output/SKILL.md) Rule 7 — never "spec tree created" without the quoted ledger. Observable result: `${specs_root}/roadmap.yaml` exists and is quoted; `${specs_root}/roadmap.md` does not. (→ spec tree)
