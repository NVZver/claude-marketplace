...
calls most — the shrunk version of Tessl's proactive spec-cache idea (`.lsa/constitution.md` §6
...
non-zero check as FAIL, and `skills/lsa/verify/SKILL.md` blocks the `GROUNDED` verdict on a
...
(that's Tessl's product, not this one's — `.lsa/constitution.md` §6 "Adjust #2").
