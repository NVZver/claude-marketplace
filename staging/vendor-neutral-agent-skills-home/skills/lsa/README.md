...
When `.lsa.yaml` is absent, LSA applies the defaults documented in [`knowledge/conventions.md`](knowledge/conventions.md) §"`.lsa.yaml` defaults": `constitution: .lsa/constitution.md`, `specs_root: .lsa/`, `mode: code`, `modules: {}`. The workspace lives entirely under `.lsa/` so you can `rm -rf .lsa/` to fully detach. See [`ARCHITECTURE.md`](ARCHITECTURE.md) §3 for the full schema.
...
LSA depends on [`core`](../core/) for fact-grounding discipline ([`core/ground-rules`](../core/ground-rules/SKILL.md)); Claude Code auto-installs it. Invoke skills directly via `/lsa:discover`, `/lsa:specify`, `/lsa:verify`, `/lsa:delegate`, `/lsa:reconcile`, or let Claude trigger by description match.
