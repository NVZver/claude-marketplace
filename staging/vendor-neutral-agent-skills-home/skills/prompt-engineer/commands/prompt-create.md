...
7. Show the full generated content per the [`../../core/output/SKILL.md`](../../core/output/SKILL.md) Rule 7 *Delivery test* (turn-final message, or carried in the approval gate's `preview`) → take approval as a labelled-option decision → **then** write the file and confirm the path — show → approve → write per Rule 7 *Authorization boundary*; on reject, write nothing
