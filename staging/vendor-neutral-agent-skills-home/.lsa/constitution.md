> **Trace.** On load, print first: `=============== [.lsa/constitution.md] [constitution] ===============`

# The Constitution

**Repo name:** `<TBD-repo-name>` — open gate, resolved before `gh repo create` (see the migration pitch's Gate decisions in the source repo, `NVZver/claude-marketplace`)
**Version:** 0.15 — migrated to the vendor-neutral home repo
**Scope:** Tech is the first pack; the **core is domain-neutral**. Substrate: **tool-agnostic** — the spec layer (EARS + Gherkin) targets no single tool; distribution is the open Agent Skills format, installable by any compliant host, and code-writing is delegated to whatever implementer the developer uses.
...
> The constitution is tool-agnostic; the mapping below is the **vendor-neutral Agent Skills implementation** — this repo's actual substrate since the vendor-neutral-agent-skills-home migration. This repo's agent instructions live in `AGENTS.md`, the vendor-neutral standard ([agents.md](https://agents.md/)); its skills conform to the open Agent Skills spec ([agentskills.io/specification](https://agentskills.io/specification)) — see `skills/core/VERIFICATION.md` §"Agent Skills spec conformance". Claude Code remains *a* reference implementation among several a compliant host may provide; nothing here assumes it's the only one.

### Primitives and where each lives

Five primitives, mapped to this repo's vendor-neutral surfaces.

| Primitive | What it is | Home |
| --- | --- | --- |
| **Rule** | Always-on constraint | `AGENTS.md` (loaded every session by any compliant host) |
| **Skill** (Knowledge) | On-demand expertise, loaded by description match | `skills/<pack>/<topic>/SKILL.md` |
| **Agent** (Actor) | A worker subprocess with declared tools | `skills/<pack>/agents/<name>.md` |
| **Command** (Actor) | A user-invoked workflow | `skills/<pack>/commands/<verb>.md` |
| **Bundle** | Versioned package of the above | A pack (`skills/<pack>/`), distributed via this repo, installable with `npx skills add <org>/<repo>` |

**Distribution: one repo, host-agnostic install.** Everything ships through this repo's root `skills/<pack>/<name>/SKILL.md` catalog. Any Agent Skills-compliant host installs it: `npx skills add <org>/<repo>` fans skills into the host's own skills directory, or a maintainer copies the skill directories manually (CONTRIBUTING.md). No build step, no per-host packaging.

**The always-on-vs-on-demand resolution.** A rule that must fire on *every* task belongs in `AGENTS.md` (genuinely always-on, read by any compliant host); a procedure needed only *sometimes* belongs in a description-matched skill. So `ground-rules` (always-on) ships as an `AGENTS.md` section; `flow-selector` ships as a skill invoked by an `AGENTS.md` rule on every non-trivial task; `actor-template` (on-demand) ships as a skill loaded on description match. This ends the activation tension — the discipline that must always apply is loaded every session, not gambled on a description match.

**The registry/lazy-load principle stays.** Read the map, enter only the rooms you need — the context-budget discipline for the whole system. Per-component token cost tracking is whatever the host reports; the load-bearing idea (lazy-load) doesn't depend on a specific host feature.
...
**Decision: RESOLVED → adopted.** See §2 sub-principle 2a (Acceptance criteria are journey-shaped) and `skills/lsa/discover/SKILL.md` User Verification 2 rows 1a + 1b; `skills/lsa/verify/SKILL.md` AC-ID trace; `skills/lsa/plan/SKILL.md` epic `**Covers:**` line. Feature: `.lsa/archive/2026-05-21-ears-journey-shape-ac/`.
...
3. **Substrate. → UPDATED (v0.15): tool-agnostic, vendor-neutral Agent Skills distribution.** The constitution targets no single tool — the spec layer is EARS + Gherkin, portable across implementers (Spec Kit, Kiro, Cursor). Distribution is the open Agent Skills format (`skills/<pack>/<name>/SKILL.md`, `AGENTS.md`, `npx skills`); code-writing is delegated out, not done in-system. Claude Code was the v0.10–v0.14 first reference implementation; v0.15 migrated the home repo itself off Claude-plugin-specific packaging while keeping Claude Code as one fully-supported host among others.
...
6. **Name. → RESOLVED (partial): file renamed.** The constitution file itself is `constitution.md` (renamed from `VISION.md`, matching GitHub spec-kit's ubiquitous term for the same role). The **repo's own name** remains `<TBD-repo-name>` — a separate, still-open gate (see the migration pitch's Gate decisions in `NVZver/claude-marketplace`).

---

## Changelog

- **v0.15** — Vendor-neutral migration. Renamed `VISION.md` → `constitution.md` (+ `VISION-digest.md` → `constitution-digest.md`), matching GitHub spec-kit's ubiquitous term for the same role. Rewrote §2 principle 9 (*Substrate-native first*) from a Claude Code-specific primitive list (`AskUserQuestion`, etc.) to a vendor-agnostic contract (labelled options + one-line outcomes; host maps to its native picker where one exists). Rewrote §3's primitives table and distribution paragraph: `AGENTS.md` (not `CLAUDE.md`) is the always-on home; skills live at `skills/<pack>/<name>/SKILL.md` (not a Claude plugin marketplace); install is `npx skills add` (not `/plugin install`), with manual copy as the documented fallback. §7 open decision 6 (Name) split: the constitution filename is resolved; the repo's own name remains a separate, still-open gate. Source: `.lsa/pitches/vendor-neutral-agent-skills-home.md` in `NVZver/claude-marketplace`, epics `repo-scaffold` through `constitution-retarget`. Historical changelog entries below (v0.1–v0.14) describe the source repo's history verbatim and are not rewritten.
- **v0.14** — §5 measurement-unit clarification. The three tracked metrics (accuracy to the task, proven facts with sources, only-required-changes) are measured **per LSA cycle, from artifacts** (`conformance.md` + gate-script output), not session-level telemetry — the original §5 wording did not state the unit of measurement, and `restore-tracked-metrics-harvest` (pitch, approved 2026-07-19) builds the harvest at the per-cycle grain. Clarification only; no metric definition, gate, or threshold changed. Applied via `lsa:revise-constitution` per the pitch's Fork 4 gate decision — constitution edits are owner-only and this one shipped on its own `constitution/reconcile-emit-guard-metrics-clarification` branch, merged into the epic branch rather than self-applied inline. Corresponds to `lsa` 0.30.0 (`reconcile-emit-guard` epic, `scripts/metrics-harvest.sh` + `reconcile`'s new emit step + `scripts/lint.sh` C17).
- **v0.13** — Deterministic-work-is-scripted principle. Added §2 principle 10 (*"Deterministic work is scripted"*): any deterministic step of meaningful complexity — enumeration, set-difference, lookup, tally, format transform — is performed by a script whose output the model cites, never recomputed at inference time; the model spends tokens on judgment. Codification only — unifies doctrine already scattered across the `.lsa.yaml` gate contract (`.lsa.yaml:13` "Pro-safe, local bash, zero model calls"), `skills/core/knowledge/fast-path-source-of-truth.md`, and `skills/manager/agents/project-manager.md` ("deterministically (zero model tokens)"). Carries the meaningful-complexity boundary (a trivial one-item check is not forced into a script — ceremony scales to weight, §3). `core/CLAUDE.md` gains a one-line card pointer to the principle (packaging only — restates no rule text); `scripts/lint.sh` gains C15, a C6-shaped presence guard that FAILs if the principle is later dropped from `.lsa/constitution.md` or the card. No new deterministic-work script lands here (those are the reconcile/verify/hygiene epics that follow). Corresponds to `core` 0.19.0 — the card edit is the only plugin surface touched; VISION.md + lint.sh are repo-level and trigger no plugin bump.
...
- **v0.6** — LSA-skill refit (credo rollout PR 2). Every LSA skill (+ `core/tier-selector` — later renamed `core/flow-selector` in v0.8) adopts a component-specific output format that satisfies the four golden rules in `core/output`. **`lsa-specify` collapses 7 confirm stops to 3 bundled gates** (Gate 1 = requirements + contract-trigger; Gate 2 = test-suites + contract + design; Gate 3 = final integration — Gates renamed "User Verification N" in v0.8). **`lsa-discover` Output becomes a 3-row table** (Module / Change / Acceptance) instead of a single-paragraph context summary; Step 2 questions (b) and (c) shift to assume-then-override. **`lsa-verify` reports lead with the verdict** (PASS / FAIL / PASS WITH WARNINGS); metadata moves below the fold. Every decision-bearing prompt uses `AskUserQuestion` in Claude Code (substrate-native) with text decision-blocks as the fallback. Corresponds to `lsa` plugin v0.4.0; `skills/lsa/ARCHITECTURE.md` Version bumped 0.2.1 → 0.4.0.
- **v0.5** — Codified the operating-philosophy credo: §0 sentence + §2 sub-principle 1a (*Ownership over automation*) + §2 principle 9 (*Substrate-native first*). The `core/ground-rules` skill extended 4 → 6 content rules (added Rule 0 Ownership + Rule 5 No filler + Rule 1 amendment Scope + Illustrative). NEW skill `core/output` ships the format golden rules every component cites — single source of truth for output discipline (canonical list at `skills/core/output/SKILL.md`; v0.5 release defined four rules, extended to five with *concrete* in `core` v0.5.0; declared marketplace-wide canonical with regression probe in `core` v0.5.5). NEW Knowledge surface `skills/core/knowledge/output-vocabulary.md` lifts the verdict vocabulary out of any Actor body. Corresponds to `core` plugin v0.4.0. The LSA-skill refit (per-component formats) lands as Vision v0.6 alongside `lsa` plugin v0.4.0.
