...
1. **Two of three declared gates never run in CI.** `.lsa.yaml:16-18` declares three gate
   ...
   *"No claim without Statement + Source + searchable quote"* (`.lsa/constitution.md:37`) — so its
...
reconcile (`.lsa/constitution.md:247`); auto-fixing (every check reports and blocks, never edits).
