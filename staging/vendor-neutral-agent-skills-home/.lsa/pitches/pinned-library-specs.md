...
`.lsa/constitution.md` §6 describes. VISION's before/after example claims discovery "records in a
...
`skills/lsa/knowledge/conventions.md` §"Library documentation protocol": check for `resolve-library-id`
...
A fast wrong answer is a defect" (`.lsa/constitution.md` §2 principle 1). (Noted for accuracy: the same
...
`.lsa/constitution.md` §6 Adjust item 2 already analyzed this and reached a verdict: "do NOT build a
...
- **Main components:**
  ...
  - A staleness check. This is the cheap part, because the drift machinery already exists and is
    ...
    (`skills/lsa/ARCHITECTURE.md` §3). A pinned library spec is the same pattern with a different watched
    artifact: not source globs, but the manifest line. Per `.lsa/constitution.md` §2 principle 10 this
    comparison is deterministic and belongs in a script the model cites.
  - One edit to `skills/lsa/knowledge/conventions.md`: pinned specs slot into the read protocol ahead of
...
1. **The read-order inversion — this is the sharpest risk.** `.lsa/constitution.md` §2 principle 6
...
3. **The pinned spec becomes a doc dump.** A full API surface is thousands of tokens loaded on
   every feature — a direct violation of `.lsa/constitution.md` §2 principle 5 (the map is not the
...
6. **No conventional dogfood target in this repo.** Verified: the repo contains zero
   ...
   dependency here to pin, and `.lsa/constitution.md` §1 makes dogfooding non-negotiable DNA. Resolved
...
1. This pitch does NOT build a spec registry of any size beyond the 3-5 cap — that is Tessl's
   product, and `.lsa/constitution.md` §6 Adjust item 2 already ruled it out.
...
3. This pitch does NOT auto-refresh or auto-rewrite a stale pinned spec. Detection only; the human
   ...
   on the human's behalf, against `.lsa/constitution.md` §2 sub-principle 1a (ownership over automation)
   and against Level 2.5's absorb-don't-automate posture.
4. This pitch does NOT replace the reactive protocol. `skills/lsa/knowledge/conventions.md` remains the
...
5. This pitch does NOT support multiple versions of one library, or a version matrix. One library,
   one pin, one version.
6. This pitch does NOT re-litigate the `.lsa/constitution.md` §6 item-2 verdict, and it does NOT reopen
