...
> `lsa:reconcile` are **both** in the floored set (`skills/lsa/knowledge/model-routing.md` §"Resolution
...
correctness gate. Its independence today is real but partial: `skills/lsa/reconcile/SKILL.md`
...
**Evidence the obvious fix does not work.** `skills/lsa/knowledge/model-routing.md` §"Resolution
...
**What is actually true today and unrecorded.** `skills/lsa/delegate/SKILL.md` — "LSA writes no
...
- **Main components:** `skills/lsa/reconcile/SKILL.md` (the verdict line + a new constraint
  defining the three values); `skills/lsa/delegate/SKILL.md` (declare the implementer identity so
  reconcile has something to compare against); `skills/lsa/knowledge/quality-gate-contract.md`
  §"Independence rule" (extend the existing observable-not-asserted rule to the model axis);
  `skills/lsa/knowledge/model-routing.md` (state explicitly that the floor takes precedence over any
  cross-model preference, closing the contradiction); the `.lsa.yaml` schema in `skills/lsa/README.md`
  and `skills/lsa/ARCHITECTURE.md` §3; plus the plugin CHANGELOG + SemVer bump per `/CLAUDE.md`
  §Discipline.
...
1. **The floor collision** — "make reconcile a different model" contradicts the shipped floor
   (`skills/lsa/knowledge/model-routing.md` §"Resolution algorithm") and its adversarial scenario.
...
3. **Coercion drift** — a blocking `require` sits uneasily with LSA's Level 2.5 advisory posture
   (`.lsa/constitution.md` §4, absorb don't forbid). Mitigation: `require` is opt-in, never the default,
...
5. **Scope creep into an eval subsystem** — "adversarial review" invites building judge harnesses.
   ...
   (`.lsa/constitution.md` §6 Adjust #3).
...
4. This pitch does NOT re-tier or unfloor `lsa:reconcile`. The floor
   (`skills/lsa/knowledge/model-routing.md` §"Resolution algorithm") is preserved and made explicitly
   dominant.
