...
- **Main components:** one new knowledge file at `skills/core/knowledge/new-plugin-checklist.md` (pure knowledge, no actor logic; the `core/knowledge/**/*.md` glob in `.lsa.yaml` already covers this path); one edit to `CONTRIBUTING.md` adding an "Adding a plugin" section between the existing "Adding a Knowledge surface" and "Editing an existing skill" sections.
