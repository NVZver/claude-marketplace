...
Small, single-skill addition. One new always-on skill `skills/core/reuse-first/SKILL.md` in
...
`core/CLAUDE.md` always-on block; and — in the same commit — a `skills/core/CHANGELOG.md` entry, a
SemVer bump in `core/.claude-plugin/plugin.json`, and README deltas (`README.md` six-plugins
area if the user-visible surface changes, `skills/core/README.md` skill list), per the per-plugin
...
- **Main components:** one new `skills/core/reuse-first/SKILL.md`; one line in
  `core/CLAUDE.md`; `skills/core/CHANGELOG.md` + `core/.claude-plugin/plugin.json` bump;
  `README.md` + `skills/core/README.md` deltas. No other plugin is touched.
