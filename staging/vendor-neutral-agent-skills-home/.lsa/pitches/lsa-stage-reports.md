...
- **Main components:** new `skills/lsa/knowledge/stage-report.md` defining the report shape — reusing the `core/output` Rule 7 compressed inspection table (`| # | file:line | type | summary |`, `core/skills/output/SKILL.md:88-95`) for the files section; one citation line added to each of the 7 skills under `lsa/skills/` and to `skills/lsa/agents/orchestrator.md` Step 5; per-plugin CHANGELOG + SemVer bump.
