...
Verdict-vocabulary decision (resolved): the verdict tags (`PROPOSED`, `DRIFT`, `PASS`, `FAIL`, `RESOLVED`, etc., from `skills/core/knowledge/output-vocabulary.md`) become **guidance**, not hard. Rationale: they are response shape, not content. Prerequisite check ships first to ensure no audit/lint tooling silently breaks — see Tasks step 1 below.
...
Small batch. The change is a re-classification inside one file (`skills/core/output/SKILL.md`) plus a sweep of cite-sites across skill bodies that currently treat the rules as hard constraints. Surface area is bounded because the rules already have a numbered structure — they're being re-tagged, not rewritten.
...
- **Main components:**
  - `skills/core/output/SKILL.md` — re-organize into two sections: (1) Hard rules — fact-grounding (Sources + Quotes, file-load trace, citation format); (2) Guidance — the remaining shape rules (Structured, Minimal, Formatted, Concrete, What-and-why preamble, Show-changes-inline) with a header note that these are outcomes to aim for, not a checklist to satisfy.
  ...
  - `skills/core/CHANGELOG.md` + version bump (likely minor — public-surface behavior change).
...
- **Tasks (ordered):**
  ...
  2. Revise `skills/core/output/SKILL.md` to split Hard rules vs. Guidance per the structure above.
