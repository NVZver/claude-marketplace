...
- Fork 4 (constitution wording): add a one-line `.lsa/constitution.md` §5 clarification via `lsa:revise-constitution` in the same cycle, so the constitution and the implementation agree on the record.
...
Make the three metrics `.lsa/constitution.md` §5 commits to real by deriving them from artifacts the
...
(`.lsa/constitution.md` §7 decision 5). None are being tracked today.
...
- `skills/lsa/CHANGELOG.md` (`[0.16.0]`, 2026-06-08) records the cause under Changed: "**`verify`** —
...
The cost is credibility, and the system already says so out loud. `skills/core/CHANGELOG.md` (`[0.20.0]`)
...
`.lsa/constitution.md` §2 principle 1, "Trust is the product", unmeasured self-assessment is the most
...
`skills/core/CHANGELOG.md` — honestly withheld. The two May rows in `.lsa/metrics.md` are stale enough
to mislead.
...
- **Main components:**
  ...
  3. A tightened `conformance.md` output contract in `skills/lsa/reconcile/SKILL.md` §Output, so
...
3. **Citation density has no honest denominator.** The true metric is *share of claims carrying a
   ...
   `.lsa/constitution.md` §2 principle 1; a flattering mislabel is not.
...
4. **Goodhart — the metric becomes a target.** An agent that knows orphan hunks are counted can
   ...
   (`skills/lsa/knowledge/quality-gate-contract.md`). Metrics inherit that boundary; they do not create a
   new one.

5. **`.lsa/constitution.md` §5's wording says "session-level".** This pitch measures per-cycle, from
...
2. This pitch does NOT introduce Wilson confidence intervals, Elo, or any variance statistic.
   `.lsa/constitution.md` §6 Adjust #3 deferred these as "genuinely overkill for v1" and this pitch does
