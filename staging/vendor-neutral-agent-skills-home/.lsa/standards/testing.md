...
Each plugin defines manual V1 / V2 / V3 probes in its `VERIFICATION.md` (or equivalent). The `core` plugin has [`skills/core/VERIFICATION.md`](../../skills/core/VERIFICATION.md); `lsa` documents its probes inside `.lsa/2026-05-20-lsa-v0.2.0-design.md` §13 until an `skills/lsa/VERIFICATION.md` is added. The probes are:
...
Run probes on a fresh session of each surface. No automated harness in this release — per `.lsa/constitution.md` §6 *"Adjust #3"*: *"Statistical eval rigor… genuinely overkill for v1, defer."*
...
Wilson CIs, Elo head-to-head ranking, variance-aware regression checks are **out of scope** for this release. Source: `.lsa/constitution.md` §6 Adjust #3 — *"The day 'did my edit make this better or worse?' becomes unanswerable with pass/fail, this is the tool. Not before."*
...
[`skills/lsa/CORE.md`](../../skills/lsa/CORE.md) §6). Source: observer feature eval,
