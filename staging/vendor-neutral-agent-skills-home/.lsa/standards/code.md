...
What follows are the standards every artifact in this repo follows. Distilled from `/CLAUDE.md` "Discipline" and `.lsa/constitution.md` §1 *"Distribution + versioning"* and `.lsa/archive/2026-05-20-core-v1/design.md` §3 *"Plugin layout"*.
...
Because there's no `/src/`, LSA runs in **doc-mode** here (`mode: docs` in `/.lsa.yaml`) — verification diffs `artifact_paths` rather than `/src/`. See `skills/lsa/ARCHITECTURE.md` §4.10 and §5 (Phase 5 — Verify).
...
Source: [`core/skills/output/SKILL.md:42`](../../skills/core/output/SKILL.md) Rule 2 *"Pull, don't push"* + [`core/skills/output/SKILL.md:79-86`](../../skills/core/output/SKILL.md) Rule 7 *Delivery test*; precedent [`lsa/skills/reconcile/SKILL.md:37-39`](../../skills/lsa/reconcile/SKILL.md) (`conformance.md` + verdict line).

## Constitution = `.lsa/constitution.md`

The configured constitution for this repo (per `/.lsa.yaml: constitution`) is `.lsa/constitution.md`, not `/CLAUDE.md`. `/CLAUDE.md` is the slimmed Claude Code entry point — it points at the constitution but is not the constitution.
