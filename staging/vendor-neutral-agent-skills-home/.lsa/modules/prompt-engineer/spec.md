...
**Plugin manifest:** [`prompt-engineer/.claude-plugin/plugin.json`](../../../prompt-engineer/.claude-plugin/plugin.json) (v0.8.3)
**Plugin README** (install, command table, rule categories): [`skills/prompt-engineer/README.md`](../../../skills/prompt-engineer/README.md)
**Per-agent behavior** (source of truth): [`skills/prompt-engineer/agents/prompt-engineer.md`](../../../skills/prompt-engineer/agents/prompt-engineer.md)
...
**Verification probes:** [`skills/prompt-engineer/VERIFICATION.md`](../../../skills/prompt-engineer/VERIFICATION.md) and [`prompt-engineer/tests/repo-anchored.md`](../../../prompt-engineer/tests/repo-anchored.md)
...
- **Versioning.** `prompt-engineer` evolves with its own SemVer + CHANGELOG (`.lsa/constitution.md` §1 *"Distribution + versioning"*). Currently v0.8.3.
- **Gate-delivery — show → approve → write (prompt-engineer v0.7.0).** Adopts `core` v0.13.0 (`.lsa/modules/core/spec.md`, Rule 7 *Authorization boundary*). `prompt-create` quotes the generated prompt content through a rendered channel, takes approval, and writes the file **only on approve** — a generated prompt is an approval-gated artifact, not written before its gate. Per `skills/prompt-engineer/commands/prompt-create.md`.
- **Markdown-only.** No `/src/`; the plugin is pure Markdown plus the JSON manifest. Per `.lsa/standards/code.md`.
- **Spec source-of-truth.** Behavior is owned by `skills/prompt-engineer/agents/prompt-engineer.md` + `prompt-engineer/commands/*.md` (Actors) and `prompt-engineer/knowledge/*.md` (rules); this module spec carries module-level invariants only — not a per-rule catalog (that's the knowledge files).
- **Separation of Concerns — self-applied.** Actors reference knowledge files and never restate their rules; the agent was de-inlined to satisfy its own doctrine (161 → 58 lines, v0.2.0). The doctrine is canonical in `skills/prompt-engineer/knowledge/separation-of-concerns.md`; a boundary violation is always HIGH.
...
- **Testability (v0.4.0).** `prompt-engineer/tests/repo-anchored.md` pins behavior to current repo files (each probe cites a `file:line` source of truth); `skills/prompt-engineer/VERIFICATION.md` holds portable probes + a falsifiable trigger threshold, mirroring `core`'s harness. The B3/B4 behavioral probes were calibrated against a fresh reviewer.
