...
> **Migration note:** this file's threat model was written for Claude Code's plugin-marketplace distribution model (`.claude-plugin/marketplace.json`, `/plugin install`, the SessionStart hook) and has **not yet been re-derived** for this repo's vendor-neutral Agent Skills distribution (`npx skills add`, no hook, no marketplace manifest). Paths below have been mechanically updated to the new layout, but the threat model's *content* — what's actually risky, what controls apply — needs a dedicated review pass before it should be trusted as accurate for this repo's real install path. Treat the specific Claude-plugin-hijacking and hook-trust sections as historical/not-yet-verified for the new distribution model, not as current guidance.

This is a **personal, open-source** project distributed as an open Agent Skills catalog. There is no
server, no hosted service, and no secret or PII handling — the entire product is
Markdown instruction files. The threat model and controls
below are scoped to exactly that (pending the re-derivation noted above).
...
[`skills/core/ground-rules/SKILL.md`](skills/core/ground-rules/SKILL.md) §1
...
[`skills/core/ground-rules/SKILL.md`](skills/core/ground-rules/SKILL.md)
...
- **`orchestrator`** (the LSA conductor the user talks to):
  [`lsa/agents/orchestrator.md:4`](skills/lsa/agents/orchestrator.md) declares
  ...
  ([`lsa/agents/orchestrator.md:43`](skills/lsa/agents/orchestrator.md)).
- **`product-manager`** (the pitch-shaping agent):
  [`skills/manager/agents/product-manager.md`](skills/manager/agents/product-manager.md)
...
([`.lsa/VISION.md:129`](.lsa/constitution.md)). The system's own autonomous write
...
edit."* ([`.lsa/VISION.md:9`](.lsa/constitution.md); resolved as Open Decision 1 —
*"Target rigor level. → RESOLVED: Level 2.5"*,
[`.lsa/VISION.md:253`](.lsa/constitution.md)).
...
- **They are advisory, human-in-the-loop checkpoints — not mechanical blocks.**
  ...
  confirms"* ([`.lsa/VISION.md:142`](.lsa/constitution.md)).
- **The system DETECTS and SURFACES drift; it absorbs rather than reverts.**
  ...
  ([`.lsa/VISION.md:144-156`](.lsa/constitution.md)). Untraced or out-of-scope
...
- **The human is the enforcement boundary.** This is a deliberate design choice
  ...
  ([`.lsa/VISION.md:156`](.lsa/constitution.md)) — **not a missing control.** If you
  need hard, mechanical prevention of code edits, that is Level 3, which this
  project intentionally does not target ([`.lsa/VISION.md:247`](.lsa/constitution.md)).
...
|---|---|---|
| Untrusted content ≠ instructions | `core/ground-rules` Rule 6 | [`skills/core/ground-rules/SKILL.md`](skills/core/ground-rules/SKILL.md) |
| Least-privilege tools | `orchestrator` / `product-manager` have no Write/Edit/Bash | [`lsa/agents/orchestrator.md:4`](skills/lsa/agents/orchestrator.md), [`skills/manager/agents/product-manager.md`](skills/manager/agents/product-manager.md) |
| No autonomous production code | LSA delegates code-writing | [`.lsa/VISION.md:129`](.lsa/constitution.md) |
| Destructive shell denied | deny-list | [`.claude/settings.json:8-10`](./.claude/settings.json) |
| Human-in-the-loop gates | Level 2.5 reconcile (detect + surface, absorb not block) | [`.lsa/VISION.md:9`](.lsa/constitution.md), [`.lsa/VISION.md:144-156`](.lsa/constitution.md) |
