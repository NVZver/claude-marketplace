...
#   - skills/core/tests/repo-anchored.md            §"D2" (output-discipline drift)
#   - skills/prompt-engineer/tests/repo-anchored.md §"A2" (actor ground-rules single def)
...
CANON="skills/core/output/SKILL.md"
fail=0
...
viol="$(printf '%s\n' "${raw}" \
  | grep -v '^$' \
  | grep -v "^\./${CANON}:" \
  | grep -v '^\./skills/core/tests/repo-anchored\.md:' \
...
leak="$(grep -rn "${needle}" skills/prompt-engineer/agents skills/prompt-engineer/commands 2>/dev/null || true)"
canon_hit="$(grep -rn "${needle}" skills/prompt-engineer/knowledge 2>/dev/null || true)"
if [[ -n "${leak}" ]]; then
  fail_line "C3 actor ground-rules list leaked into an Actor body:"
  printf '%s\n' "${leak}" | sed 's/^/        /'
elif [[ -z "${canon_hit}" ]]; then
  fail_line "C3 actor ground-rules list missing from skills/prompt-engineer/knowledge (moved/renamed?)"
else
  pass_line "C3 actor ground-rules list defined once in skills/prompt-engineer/knowledge"
fi
...
while IFS= read -r f; do
  ...
  for d in ${PLUGIN_DIRS}; do
    find "skills/${d}" -type f \( -name SKILL.md -o -path '*/agents/*.md' \) 2>/dev/null
  done | sort
)
...
while IFS= read -r f; do
  ...
  for d in ${PLUGIN_DIRS}; do
    find "skills/${d}" -type f -path '*/agents/*.md' 2>/dev/null
  done | sort
)
...
# ---------------------------------------------------------------------------
GR="skills/core/ground-rules/SKILL.md"
...
# spec's. See skills/core/VERIFICATION.md §"Agent Skills spec conformance" — the spec's
...
# separate invention. See skills/core/VERIFICATION.md §"Agent Skills spec conformance".
# ---------------------------------------------------------------------------
BODY_LIMIT=500
c9_viol=""
while IFS= read -r f; do
...
if [[ ! -f "${KIDX}" ]]; then
  ...
  disk_n="$(for d in ${SHIPPED_DIRS}; do
    find "skills/${d}/knowledge" -type f -name '*.md' 2>/dev/null
  done | grep -c . || true)"
  if [[ -z "${header_n}" ]]; then
  ...
  done < <(grep -oE '\]\(\.\./skills/[A-Za-z0-9_-]+/knowledge/[^)#]*' "${KIDX}" 2>/dev/null \
    | sed 's|^](\.\./||' | sort -u || true)
fi
if [[ -z "${c10_viol}" ]]; then
...
# ---------------------------------------------------------------------------
# C11 — VERIFICATION.md version-scope matches CHANGELOG.md. A VERIFICATION
# file that declares "Scope: vX.Y" must match the major.minor of its pack's
# latest CHANGELOG.md entry, so probe scope can't silently go stale (pitch
# eval-coverage-tracks-complexity: "the version-scope lint makes the drift a
# gate finding, not a manual catch"). Adoptable incrementally: packs whose
# VERIFICATION.md has no Scope line (or no VERIFICATION.md) are skipped.
# Retargeted from plugin.json to CHANGELOG.md — this repo ships no
# .claude-plugin/plugin.json (vendor-neutral migration; per-pack SemVer now
# lives only in each pack's CHANGELOG.md `## [X.Y.Z]` heading).
# ---------------------------------------------------------------------------
c11_viol=""
c11_checked=0
for d in ${SHIPPED_DIRS}; do
  vf="skills/${d}/VERIFICATION.md"
  [[ -f "${vf}" ]] || continue
  scope="$(grep -m1 -oE 'Scope: v[0-9]+\.[0-9]+' "${vf}" 2>/dev/null | sed 's/^Scope: v//' || true)"
  [[ -n "${scope}" ]] || continue
  c11_checked=$((c11_checked + 1))
  cl="skills/${d}/CHANGELOG.md"
  ver="$(grep -m1 -oE '^## \[[0-9]+\.[0-9]+\.[0-9]+\]' "${cl}" 2>/dev/null \
    | sed -E 's/^## \[([0-9]+\.[0-9]+)\.[0-9]+\]/\1/' || true)"
  if [[ -z "${ver}" ]]; then
    c11_viol="${c11_viol}${cl}: could not parse a SemVer version from the latest heading"$'\n'
  elif [[ "${scope}" != "${ver}" ]]; then
    c11_viol="${c11_viol}${vf}: Scope: v${scope} != CHANGELOG.md ${ver}.x"$'\n'
  fi
done
if [[ -z "${c11_viol}" ]]; then
  pass_line "C11 every VERIFICATION.md Scope line matches its CHANGELOG.md major.minor (${c11_checked} checked)"
else
...
# ---------------------------------------------------------------------------
# C12 — constitution digest exists and is fresh. .lsa/constitution-digest.md is
# generated by scripts/build-constitution-digest.sh from .lsa/constitution.md and embeds a
# source-sha256 of its input. The LSA read protocol reads the digest as the
# mandatory constitution read (skills/lsa/knowledge/conventions.md §"Read protocol"
...
# ---------------------------------------------------------------------------
DIGEST=".lsa/constitution-digest.md"
VISION=".lsa/constitution.md"
if [[ ! -f "${DIGEST}" ]]; then
  fail_line "C12 constitution digest missing: ${DIGEST} — generate it: bash scripts/build-constitution-digest.sh"
else
  ...
  if [[ -z "${embedded}" ]]; then
    fail_line "C12 ${DIGEST} carries no source-sha256 marker — regenerate: bash scripts/build-constitution-digest.sh"
  elif [[ "${embedded}" != "${current}" ]]; then
    fail_line "C12 stale constitution digest: ${DIGEST} was built from a different ${VISION} (embedded sha256 ${embedded} != current ${current}) — regenerate: bash scripts/build-constitution-digest.sh"
  else
...
# separate gate (skills/lsa/scripts/project-map-check.sh); this checks size only.
...
if [[ ! -f "${PMAP}" ]]; then
  ...
  if [[ "${pm_chars}" -gt "${PMAP_CHAR_LIMIT}" ]]; then
    fail_line "C13 ${PMAP} over budget: ~${pm_tokens} tokens (${pm_chars} chars > ${PMAP_CHAR_LIMIT}) — a map, not a catalog; keep it to directories (bash skills/lsa/scripts/project-map-build.sh)"
  else
...
# ---------------------------------------------------------------------------
METRICS_SKILL="skills/lsa/reconcile/SKILL.md"
