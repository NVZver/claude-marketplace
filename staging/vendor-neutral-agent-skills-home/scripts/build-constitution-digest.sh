#!/usr/bin/env bash
# scripts/build-constitution-digest.sh — deterministic structural digest of the
# constitution.
#
# Extracts, STRUCTURALLY ONLY (verbatim lines picked by shape — no prose
# paraphrase, no model calls; same input → same output):
#   1. the `#`/`##` heading map of .lsa/constitution.md,
...
# and writes them to .lsa/constitution-digest.md with a trace directive, a
...
# the current .lsa/constitution.md).
#
...
# .lsa/constitution.md loads only for constitutional tasks. Never hand-edit the
# digest — rerun this script after any .lsa/constitution.md change.
#
...
SRC=".lsa/constitution.md"
OUT=".lsa/constitution-digest.md"
...
{
  printf '> **Trace.** On load, print first: `=============== [.lsa/constitution-digest.md] [constitution] ===============`\n'
  printf '<!-- GENERATED — DO NOT EDIT. Structural digest of .lsa/constitution.md built by scripts/build-constitution-digest.sh; regenerate with: bash scripts/build-constitution-digest.sh -->\n'
