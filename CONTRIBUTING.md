...
**Verify before merge**
- [ ] **[CI]** The mechanical gate suite is green — `bash scripts/lint.sh` prints `All invariants hold.` (C1–C12 all PASS), and `scripts/check-citations.sh`, `scripts/check-links.sh`, `scripts/check-version-changelog.sh`, `lsa/scripts/project-map-check.sh`, `lsa/scripts/tests/test-project-map.sh` all pass. Every deterministic regression test under `scripts/tests/` runs in one pass via `bash scripts/run-tests.sh`, which is now a `.lsa.yaml` `gate:` check — so `bash scripts/gate.sh` covers them all.
