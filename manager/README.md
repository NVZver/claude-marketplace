...
| `manager:implement [epics]` | **Preview stub — execution deferred to `parallel-agent-delivery`.** Read-only: reads `.lsa/roadmap.md` and previews the last ~5 `backlog`/`not started` items with `file:line` citations plus an *indicative* parallel-vs-sequential note. Writes nothing and dispatches no implementer. The execution engine (dependency-wave planning, isolated git-worktree dispatch, per-PR gating, serialized merge, autonomy levels) is **not yet implemented** — owned by [`.lsa/pitches/parallel-agent-delivery.md`](../.lsa/pitches/parallel-agent-delivery.md). The merge half of that engine follows a defined contract — the **serialized merge + roadmap-write lock** at [`knowledge/serialized-merge.md`](./knowledge/serialized-merge.md) (Epic 1 of the pitch) — but the dispatch engine that runs it is still pending (Epic 2). Names the command surface; never claims work ran. |
