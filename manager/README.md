...
| `manager:next` | Recommend what to work on next. A plain "what's next" gets a fast-path answer in seconds — a Step 0 branch runs `scripts/roadmap-row.sh` to read only the first `backlog`/`not_started` item of the `.lsa/roadmap.yaml` ledger (zero model tokens, no whole-file read) and quotes it with a `file:line` citation, no agent dispatch (per [`../core/knowledge/fast-path-source-of-truth.md`](../core/knowledge/fast-path-source-of-truth.md)). The full project-manager dispatch (dependency/risk/value sequencing) is reserved for "recommend an order" / "what should I pick" questions. |
