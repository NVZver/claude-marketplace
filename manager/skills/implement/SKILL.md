...
5. **Converge — serialized merge, per autonomy level.**
   ...
   - **5c. Write the roadmap only after the merge.** Only the serialized-merge step writes `${specs_root}/roadmap.md` status, after the merge lands. Observable result: roadmap status written only post-merge, with the written row quoted.
