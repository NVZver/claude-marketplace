...
4. **Dispatch each wave (parallel within, sequential across).**
   ...
   - **4a. Dispatch the wave.** For the current wave, dispatch up to the concurrency cap — default 4, override via the `.lsa.yaml` `implement.concurrency` key (conservative vs. the vendor cap of ~8, per [`../../knowledge/parallel-dispatch.md`](../../knowledge/parallel-dispatch.md) §3) — one agent per epic, each in its own git worktree (`isolation: worktree`) on `feature/<epic-slug>` branched from the integration branch. Observable result: one running agent + worktree + branch per dispatched epic, never more than the cap.
