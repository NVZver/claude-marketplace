...
1. **Dispatch the project-manager and gate the row diffs — per the shared contract.** Dispatch the `project-manager` agent with an explicit intent (`intent: hygiene-check`) and run its returned gates per [`../../knowledge/roadmap-orchestration.md`](../../knowledge/roadmap-orchestration.md). The agent scans for hygiene issues per its own Steps 6-7 ([`../../agents/project-manager.md`](../../agents/project-manager.md) — the four scan conditions live there) and returns each finding as a proposed row diff — previous row + proposed row, each quoted with `file:line`. This skill presents each diff one by one via `AskUserQuestion` (approve / reject), sends approvals back via `SendMessage` continuation, and re-renders each row the agent applies and quotes. Observable result: each hygiene diff gated; approved rows applied by the agent and re-rendered inline; rejected diffs discarded.
