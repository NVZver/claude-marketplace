...
How `manager:implement` turns a set of epics into a dependency-ordered **wave plan** and dispatches one isolated agent per epic. This is the net-new layer of the `parallel-agent-delivery` dispatch engine: GitHub/git provide isolation (worktrees) and the independent-reconcile safety core (`parallel-agent-delivery/safety-core`) provides the gate + serialized merge — this file provides the **disjointness analysis and dispatch policy** none of them give.
...
- **One worktree + branch + agent + PR per epic — worktree isolation is required, not optional.** Each agent runs in its own git worktree (Claude Code `isolation: worktree` / `EnterWorktree`), on `feature/<epic-slug>` branched from the integration branch, and opens one PR. The PR is the reviewable convergence unit (prior-art C2). **A single shared working tree with convention-only file-ownership is non-conforming**: it gives no OS-level isolation, so a stray edit to a peer's "disjoint" file silently corrupts that peer (the exact risk flagged at `.lsa/observations/2026-06-17-tripanchor-manager-implement.md:24` and logged as finding C7 at `:80`). If the runtime cannot create a per-epic worktree, that epic is **not** run in the shared tree — it is held back and reported as an open item, never silently dispatched single-tree.
- **Concurrency cap, default 4 — override via the `.lsa.yaml` `implement.concurrency` key.** At most that many agents run at once; the default is conservative against the vendor cap of ~8. Epics beyond the cap queue and start as slots free.
- **Each agent runs the LSA loop** (`discover → specify → verify → delegate → reconcile`) for its epic, then the safety-core gate runs: the independent `lsa:reconcile` + the `.lsa.yaml` `gate:` checks ([`../../lsa/knowledge/quality-gate-contract.md`](../../lsa/knowledge/quality-gate-contract.md)).
- **Convergence is serialized** per [`serialized-merge.md`](./serialized-merge.md): merge only the tested SHA against the up-to-date base.
- **Propose ledgers, don't write them.** No epic agent edits a **shared ledger** — `CHANGELOG.md`, the `plugin.json` `version`, or `roadmap.yaml` (content + status). Each agent instead carries its ledger entries as *proposals* in its PR/payload (a CHANGELOG line, a version-bump intent, a roadmap delta); the serialized-merge step is the sole writer, assembling them one PR at a time (see [`serialized-merge.md`](./serialized-merge.md) §Shared-ledger lock). The disjoint-code write-set (§1) does **not** cover these append-files, so without this rule every fork integration conflicts on them (`.lsa/observations/2026-06-17-tripanchor-manager-implement.md:34` — *"the disjoint-code-file decomposition gives zero protection on the shared ledgers"*).
