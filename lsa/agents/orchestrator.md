...
- **Run spec-authoring inline; dispatch only where a fresh context is load-bearing.** `discover`, `specify`, and `verify` run in this one context — dispatching a separate sub-agent per stage would reload the CLAUDE.md floor + memory and re-read the same files each time, the token cost that makes the full flow unaffordable on Sonnet. Cross a boundary only for `delegate` (the implementer must be external) and `reconcile` (independence, below). Carry each stage's output forward; human approval per CORE §7; never guess an input.
