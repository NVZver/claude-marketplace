...
2. **`scripts/rag-query.sh "<query text>"`** (rank within scope) — within the directory `project-map.yaml` resolved, query the local RAG index before reading whole files. A fresh, good match returns cited chunks (`path:start-end` in its `{"results": [...]}` JSON, exit 0) to use directly, skipping a whole-file `Read` for that content.
