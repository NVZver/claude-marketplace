...
4. **Project map:** run the shipped builder — prefer `bash "${CLAUDE_PLUGIN_ROOT}/scripts/project-map-build.sh"` when `CLAUDE_PLUGIN_ROOT` points at the installed `lsa` plugin; otherwise `bash lsa/scripts/project-map-build.sh` from a marketplace checkout. Observable result: repo-root `project-map.yaml` exists (3-level directory tree). Skip only if the script cannot be found — note the gap; discover falls back to a tree-walk. (→ project-map.yaml)
