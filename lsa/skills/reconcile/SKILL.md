...
1. **Per-module drift diff.** For each module in `.lsa.yaml`, resolve the baseline SHA via `git log -1 --format=%H -- <spec-path>` (the last commit that modified the module's spec file), then run `git diff <baseline-sha> -- <artifact_paths>` (working-tree against the baseline SHA — **no `..HEAD` suffix**; that form misses uncommitted edits, which is the very case `vision/VISION.md:138` is designed for). Observable result: a per-module diff summary printed (file count + line count + one-line summary of what changed).
