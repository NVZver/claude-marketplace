...
4. **Per-module hard confirm.** Present each delta to the human individually. For each one ask:

   ```
   Module: <name>
   Classification: <(a) change to existing behavior / (b) new behavior>
   Proposed spec update: <one-line description>
   Apply? (y / n)
   ```
