...
2. For each user flow: confirm it is buildable on what exists. This is a broader, exploratory search — distinct from Step 1's named-symbol resolution via `scripts/resolve-refs.sh` — so query `scripts/rag-query.sh --path <project-map-resolved-dir> "<query>"` directly first, falling back to `Grep`/`Read` exactly as before on a miss, stale index, or unavailable Docker daemon, per [`knowledge/conventions.md`](../../knowledge/conventions.md) §"Read protocol" (print a one-line notice when the fallback path is used); infeasible → flag. (→ feasibility)
