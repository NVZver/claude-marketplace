...
1. Identify the modules / functions / types the spec names (your judgment — never scripted), then pass them as arguments to `bash scripts/resolve-refs.sh <symbol>…` and cite its per-symbol resolution (`exists @ file:line` | `new` | `MISSING` | `OUT-OF-RANGE`) as the reference map — instead of multiple manual searches. Resolving each symbol is deterministic lookup (`.lsa/VISION.md` §2 principle 10); the GROUNDED / NOT-GROUNDED judgment stays yours. (→ reference map)
