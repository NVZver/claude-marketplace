...
## RAG-powered search (optional)

`discover`/`verify`/`reconcile` use local vector + lexical search to find codebase facts, ranked over `Grep`/`Read` — measured 92% accuracy vs. 58% blind, at ~35K tokens vs. ~176K ([full table](../README.md#repo-search--measured-not-assumed)). Docker is the only prerequisite; everything else is unattended.

**Start from scratch.** With Docker installed and `init` already run, either wait for the one-line offer at the next session start, or run `/lsa:bootstrap-rag` directly. One invocation builds the image, indexes the repo, wires the git hook, and seeds ranking config (`rag: canonical_paths:`, auto-derived from `modules.*.artifact_paths`) — nothing else to do.

**Keep it up to date.** The wired hook re-indexes on every commit (best-effort, never blocks). CI enforces freshness independently — `rag-index-matches-head` fails the PR on a stale index, not silently. Added a new module, or want root docs covered too? Re-run `lsa/scripts/seed-canonical-paths.sh .` to refresh the ranking config; it merges in new entries without touching ones you added by hand.
...
| Skill | Purpose |
|---|---|
| **`discover`** | Extract user intent and gather the codebase facts the spec rests on — consulting the script-generated project map ([`project-map.yaml`](../project-map.yaml), 3-level directory tree) to resolve the directory a request touches, then calling `lsa/scripts/rag-query.sh --path <that-directory>` directly — a real query-time pre-filter, not a whole-repo query filtered afterward — before falling back to `Grep`/`Read` (a miss, stale index, or unavailable Docker daemon falls back exactly as before, with a one-line notice). Also the universal input-resolver other skills call. |
...
| **`verify`** | **Before** delegating: ground the spec against the codebase, and run the `.lsa.yaml` `gate:` block — citing each command + exit code (a non-zero gate blocks `GROUNDED`). Step 1 resolves the model-identified symbols via `scripts/resolve-refs.sh` (per-symbol `exists @ file:line` / `new` / `MISSING` / `OUT-OF-RANGE`) instead of multi-round `Grep`; the model still identifies which symbols the spec names and owns the `GROUNDED` verdict. Step 2's broader feasibility check — is each user flow buildable — calls `lsa/scripts/rag-query.sh --path <project-map-resolved-dir>` directly before falling back to `Grep`/`Read`, distinct from Step 1's named-symbol resolution. Output: `GROUNDED` / `NOT-GROUNDED` + `grounding.md`. |
...
| **`bootstrap-rag`** | Given a target repo with Docker installed and `init` already run: builds the plugin-shipped RAG engine image, runs the initial index, wires `core.hooksPath` to the plugin's own portable `pre-commit` hook, seeds `rag: canonical_paths:` (`lsa/scripts/seed-canonical-paths.sh`), and appends `rag-index-fresh`/`rag-index-matches-head` to `.lsa.yaml`'s `gate:` block plus a `.lsa/.rag-index/` entry to `.gitignore` — one invocation, no further manual step. A `SessionStart` hook (`hooks/rag-bootstrap-check.sh`) offers this skill automatically when Docker is reachable and the repo isn't bootstrapped yet; silent otherwise. |
