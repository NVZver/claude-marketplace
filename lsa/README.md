...
| Skill | Purpose |
|---|---|
| **`discover`** | Extract user intent and gather the codebase facts the spec rests on — consulting the script-generated project map ([`project-map.yaml`](../project-map.yaml), 3-level directory tree) to scope reads, then querying `scripts/rag-query.sh` within that scope before falling back to `Grep`/`Read` (a miss, stale index, or unavailable Docker daemon falls back exactly as before, with a one-line notice). Also the universal input-resolver other skills call. |
...
| **`verify`** | **Before** delegating: ground the spec against the codebase, and run the `.lsa.yaml` `gate:` block — citing each command + exit code (a non-zero gate blocks `GROUNDED`). Step 1 resolves the model-identified symbols via `scripts/resolve-refs.sh` (per-symbol `exists @ file:line` / `new` / `MISSING` / `OUT-OF-RANGE`) instead of multi-round `Grep`; the model still identifies which symbols the spec names and owns the `GROUNDED` verdict. Step 2's broader feasibility check — is each user flow buildable — queries `scripts/rag-query.sh` within the `project-map`-resolved scope before falling back to `Grep`/`Read`, distinct from Step 1's named-symbol resolution. Output: `GROUNDED` / `NOT-GROUNDED` + `grounding.md`. |
