...
| **`lsa-discover`** | Light three-question probe at the start of every Standard / Extended task (was `T2 / T3`). Standard oral; Extended scratch `discovery.md`. Since `lsa` v0.7.1: per-line picker is skipped when Step 1 yields a single unambiguous candidate AND the human hasn't asked for `custom` (silent-acceptance); remaining picks batch into one multi-question call. |
...
| **`lsa-reconcile`** | Absorb direct artifact edits into module specs — Level 2.5 (`vision/VISION.md:138`). One delta at a time — stop and present each individually; do not proceed without explicit approval. |
