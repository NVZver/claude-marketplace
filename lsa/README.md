...
| Skill | Purpose |
|---|---|
| **`discover`** | Extract user intent and gather the codebase facts the spec rests on — consulting the script-generated project map ([`project-map.yaml`](../project-map.yaml), 3-level directory tree) to resolve the directory a request touches, then calling `scripts/rag-query.sh --path <that-directory>` directly — a real query-time pre-filter, not a whole-repo query filtered afterward — before falling back to `Grep`/`Read` (a miss, stale index, or unavailable Docker daemon falls back exactly as before, with a one-line notice). Also the universal input-resolver other skills call. |
