...
**Knowledge** (the six rule categories): [`prompt-engineer/knowledge/`](../../../prompt-engineer/knowledge/)
...
- `core/output` Rule 7 (show-changes-inline) — the `prompt-review` command carries the **author-time** half of its enforcement (Step 3 item `l`); the PR-time half lives in `lsa:verify`. Cited directly in all three of `prompt-engineer/commands/{prompt-review,prompt-optimize,prompt-create}.md` (linking `core/skills/output/SKILL.md`).

This relationship is reciprocal: `core`'s Rule 7 invariant (`.lsa/modules/core/spec.md`) names `prompt-engineer:prompt-review` as one of its two warning-only regression checks.

## Invariants

- **Versioning.** `prompt-engineer` evolves with its own SemVer + CHANGELOG (`.lsa/VISION.md` §1 *"Distribution + versioning"*). Currently v0.4.0.
...
- **Rule categories are canonical in knowledge.** The six categories — actor ground rules (10), knowledge quality checks (6), separation of concerns (5 boundary violations), KISS/DRY (5), AI over-engineering (5), context budget (4) — live in `prompt-engineer/knowledge/*.md`. Commands reference them by path; they are not restated in Actors.
