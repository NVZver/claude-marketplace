...
**Plugin manifest:** [`manager/.claude-plugin/plugin.json`](../../../manager/.claude-plugin/plugin.json) (v0.10.0)
**Plugin README** (install, dependencies, status): [`manager/README.md`](../../../manager/README.md)
**Knowledge** (pitch format, role adaptation, epic decomposition, sequencing heuristics, command naming, roadmap orchestration): [`manager/knowledge/`](../../../manager/knowledge/)
...
- **Versioning.** `manager` evolves with its own SemVer + CHANGELOG (`.lsa/VISION.md` §1 *"Distribution + versioning"*). Currently v0.10.0.
...
- **`manager:implement` is a read-only preview stub (manager v0.10.0).** `manager:implement [epics] [--parallel|--sequential]` ([`../../../manager/skills/implement/SKILL.md`](../../../manager/skills/implement/SKILL.md)) reads `${specs_root}/roadmap.md` and previews the last X (~5) `backlog`/`not started` items with `file:line` citations plus an INDICATIVE parallel-vs-sequential note. It writes nothing and dispatches no implementer, and never implies execution ran (per `.lsa/VISION.md:15` *ownership over automation* and the `parallel-agent-delivery` pitch's "done is a gate-proven predicate"). The execution engine — dependency-wave planning, isolated git-worktree dispatch, per-PR gating, serialized merge, autonomy levels — is **not yet implemented** and is owned by the `parallel-agent-delivery` feature ([`../../pitches/parallel-agent-delivery.md`](../../pitches/parallel-agent-delivery.md)), out of scope for this stub.
