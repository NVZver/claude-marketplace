...
Aggregate one-line row per archived Extended-flow feature (was `T3`). Written by `lsa:verify` on clean PASS. Pass/fail counts only — no statistical eval per `.lsa/VISION.md` §6 *"Adjust #3"*.
