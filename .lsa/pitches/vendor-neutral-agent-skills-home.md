...
- Fork (repo name): **open — to be discussed** before create-repo epic; candidates remain `nvzver-skills` (prior recommendation), `grounded-skills`, `ownership`, `lsa-kit`, `ceremony`, or other. Vision still says Working name placeholder (`.lsa/VISION.md:5`). No silent pick.
...
Who has it: the maintainer (and any consumer) of `NVZver/claude-marketplace` — a personal agentic engineering system whose constitution already claims model- and tool-agnosticism (`.lsa/VISION.md:7`, *"Substrate: tool-agnostic"*; `.lsa/VISION.md:15`, *"model- and tool-agnostic"*) but whose **distribution and several runtime surfaces are Claude Code–native**.
...
- Vision working name is still a placeholder (`.lsa/VISION.md:5`, *"Working name: Vision (placeholder — to be named later)"*).
- Principle 9 hard-names Claude Code primitives (`.lsa/VISION.md:66`, *"`AskUserQuestion` for decisions…"*).
- **18** instructional files hard-bind `AskUserQuestion` (+ `.lsa/VISION.md` → **19** unique mention sites); `core/skills/doctor/SKILL.md` probes `~/.claude/plugins/` and `CLAUDE.md` fragment merge.
...
- **Main components:**
  ...
  - **Constitution** — carry `.lsa/VISION.md` + `.lsa.yaml` + module specs, **renaming `VISION.md` → `constitution.md`** (+ `VISION-digest.md` → `constitution-digest.md`) per spec-kit's ubiquitous term (see Gate decisions); replace Working name; rewrite principle 9 + §3 distribution paragraph to substrate-native-*generic* + Claude-as-optional-wrapper.
...
Denominator: source trees under `NVZver/claude-marketplace` excluding `dist/` unless noted. Passes: Glob inventory · `rg -l` hard-bind recount · Python independent recount. Dispatcher corrections vs shaping agent: manager knowledge **10** (not 11); AskUserQuestion **18** instructional files including `lsa/ARCHITECTURE.md` (+ VISION → 19 unique).
...
| Agents | **4** | `lsa/agents/orchestrator.md`, `manager/agents/{product,project}-manager.md`, `prompt-engineer/agents/prompt-engineer.md` |
...
| **21 knowledge files** (+ root index) | Mostly as-is; `lsa/knowledge/conventions.md` AskUserQuestion section → §B; `manager/knowledge/roadmap-orchestration.md` → §B. |
...
**AskUserQuestion hard-binds — 18 instructional files** (+ `.lsa/VISION.md` = 19 unique):

| # | Path |
| ---: | --- |
| 1–3 | `core/skills/{ground-rules,flow-selector,output}/SKILL.md` |
| 4 | `core/CLAUDE.md` (Rule 7 brief) |
| 5 | `lsa/knowledge/conventions.md` (§ AskUserQuestion convention) |
| 6 | `lsa/agents/orchestrator.md` |
| 7 | `lsa/skills/delegate/SKILL.md` (`allowed-tools`) |
| 8 | `lsa/ARCHITECTURE.md` |
| 9–13 | `manager/skills/{shape,next,check,decompose,implement}/SKILL.md` |
| 14 | `manager/knowledge/roadmap-orchestration.md` |
| 15–16 | `manager/agents/{product-manager,project-manager}.md` |
| 17 | `observer/skills/observe/SKILL.md` |
| 18 | `prompt-engineer/commands/prompt-create.md` |

**`/loop` binds — 3 files:** `observer/skills/observe/SKILL.md`, `observer/skills/verify-checkpoint/SKILL.md`, `lsa/skills/delegate/SKILL.md`. Replace with host-generic “re-invoke on change / per-cycle wake” contract (gate locked).

**Doctor / install probes:** `core/skills/doctor/SKILL.md` — replace `~/.claude/plugins`, `core@NVZver` keys, `marketplace.json` environment detect, and “merge into CLAUDE.md” with Agent Skills + AGENTS.md evidence.
