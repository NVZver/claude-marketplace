...
**Constitution:** [`.lsa/VISION.md`](../VISION.md)

---

## 0. Core idea — what every PR MUST preserve

Per [`.lsa/VISION.md:11`](../VISION.md):
...
| # | Invariant | Source |
|---|---|---|
| I1 | Fact-grounding (no claim without Source + quote) | `.lsa/VISION.md:35` |
| I2 | Spec-grounding (no artifact without a spec) | `.lsa/VISION.md:36` |
| I3 | Knowledge vs Actor separation | `.lsa/VISION.md:40, 57` |
| I4 | SoC / DRY / KISS | `.lsa/VISION.md:41` |
| I5 | Read before write | `.lsa/VISION.md:59` |
| I6 | Ceremony scales (T1/T2/T3) | `.lsa/VISION.md:114-122` |
| I7 | Reconcile (Level 2.5) | `.lsa/VISION.md:135-145` |
| I8 | Per-plugin SemVer + CHANGELOG | `.lsa/VISION.md:44, 161` |
| I9 | Dogfood — system builds itself | `.lsa/VISION.md:46` |
...
| # | Decision | Outcome |
|---|---|---|
| D1 | What to do with `.lsa/standards/agents.md` | **Delete outright.** File self-declares as a digest at 5 lines; everything is restated from VISION/ARCHITECTURE/ground-rules/tier-selector. Sweep references and redirect each to canonical source during PR 1. |
...
| `.lsa/standards/agents.md` | **Delete.** Sweep `grep -r "standards/agents.md"` references; redirect each to the canonical source named in the deleted section. |
| `.lsa/modules/lsa/spec.md` | Shrink ~144 → ~40 lines. Keep "Role", "State files", "Invariants", `.lsa.yaml` 1-line pointer. Delete per-skill catalog (`:35-99`); replace with 1-line skill list pointing to `lsa/README.md`. |
| `.lsa/modules/core/spec.md` | Shrink ~67 → ~25 lines. Delete per-skill behavioral-contract tables (`:17-42`). |
...
| `.lsa/standards/code.md` | Audit for VISION duplication. Trim if found. |
| `.lsa/standards/testing.md` | Audit for skill-body duplication. Trim if found. |
...
| `core/skills/tier-selector/SKILL.md` | Steps 1+2: cite `.lsa/VISION.md:124, 126` instead of restating boundary signals + classification table (`:21-37`). Resolves debt at `lsa/ARCHITECTURE.md:459`. |
...
| I3 | Knowledge vs Actor | ✓ **strengthened** | Current state has 11 boundary violations (round-2 findings). PR 2 extracts Knowledge from Actor bodies into `lsa/knowledge/conventions.md`; PR 1 strips Knowledge restatements from meta-docs. Directly serves `.lsa/VISION.md:57` *"Boundary violations are the highest-severity defect"*. |
...
| I6 | Ceremony T1/T2/T3 | ✓ | Tier-selector still classifies. Downstream skills still gate. No tier flow changes. Tier table lives in one place (`.lsa/VISION.md` §4); everywhere else cites. |
...
| I8 | Per-plugin SemVer + CHANGELOG | ✓ | Every PR includes version bump + CHANGELOG entry per `.lsa/VISION.md:44`. PR 1 → 0.2.1; PR 2 → 0.3.0; PR 3 → 0.3.1. |
