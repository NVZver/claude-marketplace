...
**Goal:** Ship `core` v0.2.0 (adds `tier-selector` skill + always-on tier fragment) and `lsa` v0.2.0 (closes seven Vision-alignment gaps), then bootstrap this repo's `.lsa/` tree so LSA actually runs against itself end-to-end.

**Architecture:** Two plugins evolve independently with their own SemVer + CHANGELOG. Core ships first (its v0.2.0 is consumed by lsa v0.2.0). Then lsa v0.2.0. Then the this-repo bootstrap as the dogfood probe. Per-plugin SemVer + CHANGELOG per `.lsa/VISION.md` §1 *"Distribution + versioning"*.
...
| `/.lsa.yaml` | Create | C | This repo's config — points at .lsa/VISION.md and .lsa/ |
| `.lsa/main.spec.md` | Create | C | Module index, cross-plugin contracts, NFRs |
| `.lsa/standards/code.md` | Create | C | Markdown-only, SemVer + CHANGELOG, plugin layout |
| `.lsa/standards/testing.md` | Create | C | Manual VERIFICATION.md probes per plugin |
| `.lsa/standards/agents.md` | Create | C | Gate types, escalation rules |
| `.lsa/roadmap.md` | Create | C | From VISION §6 / §7 / post-0.2.0 follow-ups |
| `.lsa/research-backlog.md` | Create | C | Empty table |
| `.lsa/modules/core/spec.md` | Create | C | Three skills with behavioral contracts |
| `.lsa/modules/lsa/spec.md` | Create | C | Eight skills, eight numbered phases (Phase 0 + Phases 1–7) plus ad-hoc Reconcile, gates |
| `.lsa/archive/2026-05-20-core-v1/design.md` | Relocate | C | Moved from `.lsa/archive/2026-05-20-core-v1/design.md` |
| `.lsa/archive/2026-05-20-core-v1/tasks.md` | Relocate | C | Moved from `.lsa/archive/2026-05-20-core-v1/tasks.md` |
...
**Phase C — This-repo bootstrap** (Tasks 26–34). Creates `/.lsa.yaml`, the `.lsa/` tree, slims `/CLAUDE.md`, relocates archive content. End of Phase C: `/lsa:verify` succeeds on a fresh feature branch in this repo.
...
- [ ] **1. Read the source.** Open `.lsa/VISION.md` §4 to extract the boundary signals (lines 122–134) and worked examples (the four-row table).
...
- [ ] **1. Land the body.** Five-section actor shape. Steps must contain the six-step procedure from design §4.4:
  ...
  5. On confirm — **reverse-sync per Vision §4 (`.lsa/VISION.md:143`):**
  ...
  Constraints:
  - Never block/revert/reformat the artifact edits themselves (Vision §4 `.lsa/VISION.md:144`).
...
- [ ] **4. Reject the proposal** (to avoid polluting the spec tree before Phase C lands `.lsa/modules/core/spec.md`). Expected: skill exits without modifying the spec.
...
- [ ] **7. Update status line at top** from "0.1.0 — Installable; pending stress test" to "0.2.0 — Vision-aligned; dogfooded on claude-marketplace itself; see .lsa/2026-05-20-lsa-v0.2.0-design.md".
...
```bash
git add .lsa.yaml
git commit -m "feat(repo): add .lsa.yaml — constitution=VISION.md, specs_root=.lsa/, mode=docs"
```

---

### Task 27 — Create `.lsa/main.spec.md`

**Inputs:** Task 26 complete.
**Outputs:** `.lsa/main.spec.md` exists with Module Index, Cross-Plugin Contracts, NFRs.
**Files:** Create `.lsa/main.spec.md`.
**Verification:** `grep -c "^## " .lsa/main.spec.md` prints `>= 3`.
**Spec trace:** §11.
...
- [ ] **2. Verify.** `grep -c "^## " .lsa/main.spec.md`. Expected: `>= 3`.

- [ ] **3. Commit.**

```bash
git add .lsa/main.spec.md
git commit -m "feat(specs): add main.spec.md — module index, cross-plugin contracts, NFRs"
```

---

### Task 28 — Create `.lsa/standards/{code,testing,agents}.md`

**Inputs:** Task 27 complete.
**Outputs:** Three standards files exist.
**Files:** Create `.lsa/standards/code.md`, `.lsa/standards/testing.md`, `.lsa/standards/agents.md`.
**Verification:** `ls .lsa/standards/ | wc -l` prints `>= 3`.
**Spec trace:** §11.

- [ ] **1. Create directory.** `mkdir -p .lsa/standards`.

- [ ] **2. Write `code.md`.** Content: markdown-only deliverables (no `src/`); per-plugin SemVer + CHANGELOG (Keep a Changelog); plugin layout from `.lsa/archive/2026-05-20-core-v1/design.md:33`; `${CLAUDE_PLUGIN_ROOT}` convention; bump-version-with-changelog rule from `/CLAUDE.md`.
...
- [ ] **4. Write `agents.md`.** Content: gate types (hard confirm = stop completely; soft confirm = present + correct + delegate) from `lsa/ARCHITECTURE.md:187`; escalation rules from VISION §4 (orchestrator proposes tier, human confirms or overrides); the eight first principles from VISION §2; the reconcile loop is absorptive not blocking per `.lsa/VISION.md:144`.

- [ ] **5. Verify and commit.**

```bash
git add .lsa/standards/
git commit -m "feat(specs): add standards/{code,testing,agents}.md"
```

---

### Task 29 — Create `.lsa/roadmap.md` and `research-backlog.md`

**Inputs:** Task 28 complete.
**Outputs:** Both files exist.
**Files:** Create `.lsa/roadmap.md`, `.lsa/research-backlog.md`.
**Verification:** `test -f .lsa/roadmap.md && test -f .lsa/research-backlog.md` exits 0.
**Spec trace:** §11.
...
```bash
git add .lsa/roadmap.md .lsa/research-backlog.md
...
**Outputs:** `.lsa/modules/core/spec.md` and `.lsa/modules/lsa/spec.md` exist.
**Files:** Create both.
**Verification:** `grep -c '^## ' .lsa/modules/core/spec.md` and same for lsa each print `>= 3`.
**Spec trace:** §11.

- [ ] **1. Create directories.** `mkdir -p .lsa/modules/core .lsa/modules/lsa`.
...
**Outputs:** `.lsa/archive/2026-05-20-core-v1/{design.md,tasks.md}` exist; the originals at `.lsa/archive/2026-05-20-core-v1/design.md` and `.lsa/archive/2026-05-20-core-v1/tasks.md` no longer exist. All citations to the old paths in `.lsa/2026-05-20-lsa-v0.2.0-design.md` and `.lsa/plans/2026-05-20-lsa-v0.2.0-plan.md` are updated to the new archive paths.
**Files:** Move two files via `git mv`; modify the two lsa v0.2.0 docs.
**Verification:** Both relocated files exist. `grep -rn ".lsa/archive/2026-05-20-core-v1/design.md\\|.lsa/archive/2026-05-20-core-v1/tasks.md" vision/` returns no matches (all references updated). The relocated plan's `Source spec:` line points at the new design path.
**Spec trace:** §11.

- [ ] **1. Create archive directory.** `mkdir -p .lsa/archive/2026-05-20-core-v1`.

- [ ] **2. Move design file.** `git mv .lsa/archive/2026-05-20-core-v1/design.md .lsa/archive/2026-05-20-core-v1/design.md`.

- [ ] **3. Move plan file.** `git mv .lsa/archive/2026-05-20-core-v1/tasks.md .lsa/archive/2026-05-20-core-v1/tasks.md`.
...
sed -i.bak \
  -e 's|.lsa/archive/2026-05-20-core-v1/design.md|.lsa/archive/2026-05-20-core-v1/design.md|g' \
  -e 's|.lsa/archive/2026-05-20-core-v1/tasks.md|.lsa/archive/2026-05-20-core-v1/tasks.md|g' \
  .lsa/2026-05-20-lsa-v0.2.0-design.md \
  .lsa/plans/2026-05-20-lsa-v0.2.0-plan.md
rm .lsa/2026-05-20-lsa-v0.2.0-design.md.bak .lsa/plans/2026-05-20-lsa-v0.2.0-plan.md.bak
```
...
- [ ] **5. Fix the archived plan's `Source spec:` link.** Inside `.lsa/archive/2026-05-20-core-v1/tasks.md`, the first prose line points at `../specs/2026-05-20-core-v1-design.md` (relative from the old `.lsa/plans/` location). After relocation it should be `./design.md` (sibling in the same archive directory).

```bash
sed -i.bak \
  's|../specs/2026-05-20-core-v1-design.md|./design.md|g' \
  .lsa/archive/2026-05-20-core-v1/tasks.md
rm .lsa/archive/2026-05-20-core-v1/tasks.md.bak
```

- [ ] **6. Verify.**
  - `test -f .lsa/archive/2026-05-20-core-v1/design.md && test -f .lsa/archive/2026-05-20-core-v1/tasks.md && echo OK` → `OK`.
  - `grep -r ".lsa/archive/2026-05-20-core-v1/design.md\\|.lsa/archive/2026-05-20-core-v1/tasks.md" vision/ CLAUDE.md core/ lsa/ 2>/dev/null` → no matches (all references updated).
  - `grep -c "./design.md" .lsa/archive/2026-05-20-core-v1/tasks.md` → `>= 1`.

- [ ] **7. Commit.**

```bash
git add .lsa/ .lsa/plans/
git commit -m "refactor(specs): relocate core v1 design+plan into archive; fix cross-references"
```

Note: `.lsa/plans/` may now be empty. Leave the directory in place for future feature work — its purpose persists.
...
- [ ] **2. Rewrite as the thin entry.** Sections:
  ...
  - **Constitution pointer** — *"Operating rules live in [`.lsa/VISION.md`](./.lsa/VISION.md). LSA configuration is at [`/.lsa.yaml`](./.lsa.yaml)."*
...
Per design §13 V3a: **edit an artifact, not a spec file.** Standards files (`.lsa/standards/*`) live under `${specs_root}` — they're spec content, not artifacts. Doc-mode verify diffs `artifact_paths` against the matching module spec; editing a spec file isn't an artifact change.
...
- [ ] **7. Confirm no metrics file was written.** Per design §9 (revised), T2 does not emit metrics. `find .lsa/archive -name metrics.md -newer .lsa.yaml` returns no new file.
...
- [ ] **14. Confirm absorption.** Expected:
  - The contradicted line (or wording) in `.lsa/modules/core/spec.md`'s coverage of `ground-rules` is **edited in-place** with the new value, tagged `<!-- reconciled: 2026-05-20 -->`.
...
- [ ] **15. Inspect state files.**
  ...
  - `git diff .lsa/modules/core/spec.md` → shows in-place edit (no new heading) with the `reconciled:` tag.
...
- **Task 33 V3a — standards/ artifact_paths edge case.** Standards files in `${specs_root}/standards/` aren't currently in any module's `artifact_paths`. The probe explicitly surfaces this and asks the implementer to decide. Likely resolution: add `.lsa/standards/**` to `lsa.artifact_paths` (since lsa owns the standards convention via ARCHITECTURE.md §4.2).
