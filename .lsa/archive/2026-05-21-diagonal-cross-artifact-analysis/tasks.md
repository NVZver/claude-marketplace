...
Per `.lsa/features/diagonal-cross-artifact-analysis/design.md` §"Technical Approach". The check logic is described in prose inside the SKILL body (per `lsa` invariant: *"Markdown + small JSON / YAML / bash surface. No /src/"* — `.lsa/modules/lsa/spec.md:30`). Insertion point: after `lsa/skills/lsa-specify/SKILL.md:154` (the existing AC-coverage line), before the rendered presentation of `test-suites.md`.
...
| E2E (V3 manual probe) | Re-run Journey 1, Journey 2, Journey 3 against a synthetic feature spec (a fake `.lsa/features/probe-feature/` with deliberate contradictions in Path 1 of Journey 3) and inspect the rendered Gate 2 output. Verify: (a) 4-row table appears, (b) all rows cite `file:line`, (c) failing rows render Rule 6 decision blocks, (d) approval blocks until resolved. | Must |
...
- [ ] No code smells per `.lsa/standards/code.md` (which for `lsa` reduces to: prose follows `core/actor-template` shape).
...
Add one Invariant line to `.lsa/modules/lsa/spec.md` § Invariants documenting the 4-row diagonal coverage at `lsa-specify` Gate 2 (mapping to feature AC5).

#### Scope

- **Files touched:** `.lsa/modules/lsa/spec.md` only.
...
> **Gate 2 diagonal coverage.** `lsa-specify` Gate 2 renders a 4-row cross-artifact coverage table (AC→Journey, Journey→Design, Design→Contract, Contract→test-suites). Each row cites two artifact lines; `✗` rows surface as Rule 6 decision blocks. Per `lsa/skills/lsa-specify/SKILL.md:154` (Step 5) and `.lsa/archive/<sync-date>-diagonal-cross-artifact-analysis/requirements.md`.

#### Acceptance Criteria

- [ ] **AC2.1:** `.lsa/modules/lsa/spec.md` § Invariants contains a one-bullet entry naming the 4-row diagonal coverage check (mapping to feature AC5).
...
Bump `lsa` plugin SemVer per `.lsa/main.spec.md:30` NFR3 ("every plugin maintains its own CHANGELOG.md plus a SemVer in plugin.json; bump version in same commit as changelog entry"). Add CHANGELOG entry. Update `lsa/README.md` skill table only if user-visible delta (the lsa-specify row description widens but skill name unchanged — minor edit). Sync `.lsa/main.spec.md` module index version from the stale `lsa v0.2.0` (logged as Finding #3) to the new `v0.2.x`.
...
- **Files touched:**
  ...
  - `.lsa/main.spec.md` (module index version cell)
- **Creates / modifies / deletes:** all modifies.
- **Does NOT touch:** `lsa/skills/`, `.lsa/modules/`, any feature spec.
...
### Added
- `lsa-specify` Gate 2 now renders a 4-row diagonal cross-artifact coverage table (AC→Journey, Journey→Design, Design→Contract, Contract→test-suites). Failing rows surface as Rule 6 decision blocks that block approval until resolved. Source: `.lsa/archive/<sync-date>-diagonal-cross-artifact-analysis/`.
```
...
- [ ] **AC3.4:** `.lsa/main.spec.md` module index shows `lsa` at `v0.2.2`, closing Finding #3.
...
- [ ] `lsa-sync` completed — `.lsa/features/diagonal-cross-artifact-analysis/` archived to `.lsa/archive/2026-05-21-diagonal-cross-artifact-analysis/`; module spec absorbed E2's Invariant edit; `.lsa-sync-state.json` updated.
...
| Check | Result | Reason |
|-------|--------|--------|
| Traceability | PASS | E1 maps to feature F1/F2/F3/F4/F5 + NF1/NF2 + AC1–AC4. E2 maps to feature AC5. E3 maps to `.lsa/main.spec.md:30` NFR3 + closes Finding #3. |
| Accuracy | PASS | E1 implements `design.md` §"Technical Approach" + §"Coverage table rows" + §"Citation format" + §"Rule 6 decision block on failure". E2 implements `design.md` §"Modules Affected" (lsa) and OQ5. E3 implements per-plugin discipline per `.lsa/main.spec.md:30`. |
| Consistency | PASS | E1 touches `lsa/skills/lsa-specify/SKILL.md` only. E2 touches `.lsa/modules/lsa/spec.md` only. E3 touches plugin.json + CHANGELOG + README + main.spec.md. Zero overlap. No contradictions. |
