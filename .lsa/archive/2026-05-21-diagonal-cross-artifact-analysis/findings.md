...
### Finding #3 — `.lsa/main.spec.md` module index stale
...
### Finding #4 — `.lsa/modules/lsa/spec.md` stale on three lines
...
- **Recommended fix:** Add a worked example for each ambiguous row, either in `lsa/knowledge/conventions.md` or as a sample in `.lsa/features/diagonal-cross-artifact-analysis/` itself.
...
- **Symptom:** Feature AC5 says to add the diagonal coverage documentation to `.lsa/modules/lsa/spec.md` § Invariants. But the module spec preamble says *"Per-skill behavior (source of truth per skill): lsa/skills/*/SKILL.md"* — skill behavior is intentionally NOT in the module spec.
...
- **Symptom:** Neither `lsa/CHANGELOG.md` nor `core/CHANGELOG.md` is listed in any module's `artifact_paths` in `.lsa.yaml:9-26`. Per `.lsa/main.spec.md:30` NFR3, the CHANGELOG must be bumped per feature — but docs-mode `lsa-verify` cannot see CHANGELOG edits because they fall outside `artifact_paths`.
...
- **Pragmatic resolution this session:** wrote metrics.md to the **dated** form (`.lsa/archive/2026-05-21-diagonal-cross-artifact-analysis/metrics.md`) per ARCHITECTURE convention.
...
### Finding #8 — `.lsa/main.spec.md` core row also stale
...
- **Per-plugin SemVer discipline held.** SemVer bump + CHANGELOG entry landed in the same commit (E3) per `.lsa/main.spec.md:30` NFR3.
