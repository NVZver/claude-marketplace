...
**Description.** Author the new sub-principle 2a under `.lsa/VISION.md` §2 principle 2 (*"Two groundings, always"*) and append the RESOLVED marker to §6 Adjust #1. Per Gate 3 routing decision, route via `lsa-revise-constitution`, not direct edit.

**Scope.**
- Files/modules touched: `.lsa/VISION.md`
...
**Technical Details.** Invoke `lsa-revise-constitution` with the principle 2a text and §6 RESOLVED marker text from `design.md` §5-§6. Skill is the conventional path for constitution edits (`.lsa/VISION.md` §2 principle 7: *"The human owns intent; the system absorbs reality"* — constitution edits are intent-owned, route through the dedicated skill).

**Acceptance Criteria.**
- [ ] AC1: `.lsa/VISION.md` §2 contains sub-principle 2a *"Acceptance criteria are journey-shaped"* with the body text from `design.md` §5.
- [ ] AC2: `.lsa/VISION.md` §6 Adjust #1 has the RESOLVED line from `design.md` §6.
...
- [ ] No code smells per `.lsa/standards/code.md`

---

### Epic 2: `lsa-specify` Gate 1 template + Gate 2 diagonal rows

**Description.** Update `lsa/skills/lsa-specify/SKILL.md` in two places: Gate 1 template (AC sub-block becomes EARS-form with `.lsa/VISION.md:201` citation) and Gate 2 diagonal coverage table (insert rows 1a EARS-pattern + 1b journey-shape between existing rows 1 and 2; failing-row Rule 6 prose updated to include the new resolutions).
...
**Technical Details.** Per `design.md` §1-§2. Reuses the existing `✗` Rule 6 decision-block render at `SKILL.md:162-176` for the two new rows. EARS pattern list + journey-shape definition are cited from `.lsa/VISION.md:201` and `.lsa/VISION.md` 2a (added in E1) — not restated in the SKILL body per NFR4 (Knowledge vs Actor).

**Acceptance Criteria.**
- [ ] AC1: Gate 1 template at `lsa/skills/lsa-specify/SKILL.md:48-77` shows the `## Acceptance Criteria` sub-block in EARS form with a `.lsa/VISION.md:201` citation inline.
...
**Description.** Add the `lsa` module spec invariant entry; reconcile `.lsa/roadmap.md` (mark EARS shipped, mark stale diagonal entry shipped, add v0.6.0 and v0.5.0 to Recently merged); author `lsa/CHANGELOG.md` `[0.6.0] - 2026-05-21` entry; bump `lsa/.claude-plugin/plugin.json` version `0.5.0 → 0.6.0`; review `lsa/README.md` for user-visible delta.

**Scope.**
- Files/modules touched: `.lsa/modules/lsa/spec.md`, `.lsa/roadmap.md`, `lsa/CHANGELOG.md`, `lsa/.claude-plugin/plugin.json`, `lsa/README.md` (read-only confirm)
...
**Technical Details.** Per `design.md` §7-§8. Module spec invariant text from §7. Roadmap edits from §8. CHANGELOG entry follows Keep a Changelog format per `.lsa/main.spec.md` NFR3. SemVer bump in the same commit as the CHANGELOG entry per NFR3.

**Acceptance Criteria.**
- [ ] AC1: `.lsa/modules/lsa/spec.md` has a new invariant bullet per `design.md` §7.
- [ ] AC2: `.lsa/roadmap.md` row 9 (EARS) status → `shipped — lsa v0.6.0`; row 11 (Diagonal) status → `shipped — lsa v0.5.0`; Recently merged gains two new entries.
...
- [ ] `lsa-sync` completed (feature → module specs absorbed; feature archived under `.lsa/archive/`)
- [ ] PR to `main` created
