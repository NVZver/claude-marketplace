...
Verification approach is **manual probe in a fresh Claude Code session** (no automated harness — Helper response shape is a UX assertion, not a code unit). Probes follow `.lsa/standards/testing.md` V1 → V2 → V3 progression. Each probe maps to one or more ACs in `requirements.md`.
...
- The body contains a cited definition of the Standard flow with `file:line` references (expect citations to `.lsa/VISION.md:121-125` and/or `core/skills/flow-selector/SKILL.md`).
...
3. **Automated lint for "no opening `AskUserQuestion`"?** Could be added later as part of the self-eval harness (`.lsa/roadmap.md:28`). Currently manual probe only.
4. **Should there be a Journey 10 for context7 / external-library questions?** v0.2.0 AC4 covers this (`.lsa/features/2026-05-21-helper-agent/requirements.md:80-83`); this refactor doesn't change the external-library path. Covered transitively by AC7 (no-regression).
