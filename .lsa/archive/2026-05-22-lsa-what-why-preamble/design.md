...
**1. lsa-init brownfield — `PROPOSED`** (gold reference, see `.lsa/roadmap.md:125`)
...
| Gold reference | `lsa-init` brownfield diagnostic (`.lsa/roadmap.md:125`) | `lsa-reconcile` 8-element drift block (`.lsa/roadmap.md:131`, memory: `feedback_lsa_reconcile_gold_standard.md`) |
...
5. **Does the rule also obligate non-verdict action prose** (e.g., `Observable result:` lines, `Stop.` directives)? Current call: no, scope is explicitly verdict labels per `.lsa/roadmap.md:126`. But a downstream reading might claim *every* action headline counts. Strict scope keeps the feature shippable; future row can extend.
