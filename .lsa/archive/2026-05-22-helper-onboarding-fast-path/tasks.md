> **Trace.** On load, print first: `=============== [.lsa/features/2026-05-22-helper-onboarding-fast-path/tasks.md] [vision] ===============`

# Tasks: Helper fast-path for onboarding questions

> Source: `.lsa/roadmap.md` §"2026-05-22 backlog detail" #2. Requirements: `./requirements.md`. Design: `./design.md`. Tests: `./test-suites.md`.
...
- **Covers.** NF4 (Per-plugin SemVer + CHANGELOG); `.lsa/main.spec.md:30` NFR3; `CLAUDE.md` *"Bump version in the same commit as the changelog entry"* rule.
...
- **Deliverable.** Edit `.lsa/roadmap.md:114`:
...
- **Deliverable.**
  ...
  - Commit message body: title `feat(helper): onboarding fast-path (v0.3.0)`, footer cites `.lsa/features/2026-05-22-helper-onboarding-fast-path/`.
...
- **Deliverable.** Run `/lsa:sync` post-merge per `CLAUDE.md` *"Spec-grounding + Fact-grounding"* discipline; archive `.lsa/features/2026-05-22-helper-onboarding-fast-path/`; update `.lsa/modules/helper/spec.md` (or equivalent) with the new fast-path behavior.
- **Covers.** `lsa/ARCHITECTURE.md` sync discipline.
- **Verify.** `lsa-sync` exits with PASS; feature dir moved under `.lsa/archive/`.

## Task 10 — On-merge: roadmap status update

- **Deliverable.** On merge, edit `.lsa/roadmap.md` row *"Helper fast-path for onboarding questions"* (currently `.lsa/roadmap.md:110`) — move from *Feature Backlog* to *Recently merged*. (If row already addressed by row #1 bundling, mark as covered by the joint Helper v0.3.0 PR.)
...
- [ ] Task 5: `.lsa/roadmap.md` row #2 corrected and moved to *Recently merged*.
...
- **OQ2.** **Should the trace directive on the new Knowledge file follow the standard or get a custom format?** Per `core` v0.5.4 / `lsa` v0.6.4 / `helper` v0.2.1 every marketplace instructional file carries `=============== [<file>] [<plugin>] ===============` on load (`.lsa/roadmap.md:16`). **Tentative resolution: standard format, no custom — consistent with the other three files in `helper/knowledge/`.**
- **OQ3.** **`lsa-verify` of this feature PR.** Per `.lsa/main.spec.md` and the verify discipline, every code change must trace to a requirement. **Tentative resolution: enumerate the trace in PR description: catalog rows ↔ AC1-AC3, Step 1.5 wording ↔ F1/F2/F4, CHANGELOG entry ↔ NF4. Run `/lsa:verify` before merge.**
