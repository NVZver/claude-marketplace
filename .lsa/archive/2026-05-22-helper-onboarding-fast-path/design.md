> **Trace.** On load, print first: `=============== [.lsa/features/2026-05-22-helper-onboarding-fast-path/design.md] [vision] ===============`

# Design: Helper fast-path for onboarding questions

> Source: `.lsa/roadmap.md` §"2026-05-22 backlog detail" #2 (`.lsa/roadmap.md:110-114`). Requirements: `./requirements.md`.
...
| `vision` (roadmap) | Modify | `.lsa/roadmap.md` — correct `helper/skills/helper/SKILL.md` reference in row #2 to `helper/agents/helper.md`; mark the row shipped on merge. |
...
| `.lsa/roadmap.md:114` | *"Pattern classifier lives in `helper/skills/helper/SKILL.md`."* | Replace with *"Pattern classifier lives in `helper/knowledge/onboarding-fast-path.md`; Step 1.5 in `helper/agents/helper.md` invokes it."* |
...
- **OQ1.** **Should the catalog mark certain rows as "Helper's preferred starter answer" vs. "any reasonable phrasing"?** Concretely: row 2 (*"get started with LSA"*) is THE golden test from the roadmap row (`.lsa/roadmap.md:113`). Should it carry a `priority: golden` tag so test-suites pins it as the canonical regression? **Tentative resolution: yes — single `priority` column added to the catalog table, values `golden | standard`; golden rows must each have a dedicated `test-suites.md` Journey.**
