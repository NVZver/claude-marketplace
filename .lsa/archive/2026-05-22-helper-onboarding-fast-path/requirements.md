...
| F6 | **Catalog is data, not code.** The trigger pattern list, the README-excerpt mapping table, and the negative examples shall live in `helper/knowledge/onboarding-fast-path.md` as a Knowledge file (per `.lsa/main.spec.md:32` NFR5 Knowledge vs Actor separation). The agent body holds only the "when to consult the catalog" directive. Adding a new onboarding pattern shall not require editing `helper/agents/helper.md`. | Must |
...
| NF3 | **Fact-grounding** (per `.lsa/main.spec.md:28` NFR1). Every README excerpt rendered by the fast-path carries its `file:line-range` citation. |
| NF4 | **Per-plugin SemVer + CHANGELOG** (`.lsa/main.spec.md:30` NFR3). `helper` plugin bumps minor version (v0.2.1 → v0.3.0) in the same commit; CHANGELOG entry added. |
| NF5 | **Knowledge vs Actor separation** (`.lsa/main.spec.md:32` NFR5). New file `helper/knowledge/onboarding-fast-path.md` is Knowledge (trigger patterns + excerpt catalog + negative examples). The new Step 1.5 in `helper/agents/helper.md` is Actor (when + how to consult the catalog). |
| NF6 | **No regressions on existing AC paths.** All Acceptance Criteria of the parent helper-agent spec (`.lsa/features/2026-05-21-helper-agent/requirements.md:64-99` — AC1 through AC8) shall continue to hold. AC1 (Inline question) gains a *faster* path for onboarding subset; the slow path remains for everything else. |
...
- **Input.**
  ...
  - In-repo READMEs referenced by the catalog: `README.md`, `core/README.md`, `lsa/README.md`, `helper/README.md`, plus `.lsa/VISION.md` §0 / §1 / §4.
- **Output.**
...
- **NFR1 fact-grounding** (`.lsa/main.spec.md:28`).
- **NFR5 Knowledge vs Actor separation** (`.lsa/main.spec.md:32`).
- **Vision Principle 9 — substrate-native** (`.lsa/VISION.md:66`) — closing question uses `AskUserQuestion`, never text `[a]/[b]/[c]`.
- **Vision §2 sub-principle 2a — journey-shape AC** (`.lsa/VISION.md:59`).
...
- **The full Helper refactor (roadmap row #1).** This spec adds one specific short-circuit path. The deeper assistant-shape refactor (goal-understanding, no-mandatory-picker) is `.lsa/features/<future>` and may subsume parts of this work later. Defined boundary below in **Interaction with roadmap row #1**.
...
- **First-run onboarding wizard.** Already out of scope per parent spec (`.lsa/features/2026-05-21-helper-agent/requirements.md:60`); not reopened here.
...
Journey-shaped per `.lsa/VISION.md` §2 sub-principle 2a. EARS form per `.lsa/VISION.md:204`.
...
- **OQ2.** **What counts as "in a top-level README"?** The four canonical ones (`README.md`, `core/README.md`, `lsa/README.md`, `helper/README.md`) are unambiguous. Should `.lsa/VISION.md` §0 *"The one sentence"* qualify? It is at the constitution level, not a README, but it answers *"what is this whole thing"* in one paragraph. **Tentative resolution: include `.lsa/VISION.md:13-15` as an onboarding excerpt for the *"what is this marketplace"* trigger.**
