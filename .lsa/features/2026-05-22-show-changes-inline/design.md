...
> Requirements: `.lsa/features/2026-05-22-show-changes-inline/requirements.md`.
...
*Reason: lands the `core/output` Rule 6 + LSA sweep per `.lsa/features/2026-05-22-show-changes-inline/tasks.md` step 1-2. Source: `.lsa/roadmap.md:128-132`. Type: `batch` (`add` + `edit` mix).*
...
`.lsa/roadmap.md:122-126` (row #4) wants a *what-LSA-is-doing-and-why-it-matters* preamble before every verb-headline. This row (#5) wants a *quote-the-actual-content* echo after every write.
...
**Proposal (not unilateral).** Propose combining the LSA-sweep portion of PR-β with row #4's sweep if both rows reach implementation in the same cycle. Confirm with row #4 implementor; row #4's spec (`.lsa/features/2026-05-22-lsa-what-why-preamble/design.md` §"Interaction with roadmap row #5") currently prefers independent sequencing — *"Row #4 ships first because (a) it has fewer affected sites, (b) the gold reference is already concrete, (c) the format rule is short."* Reasoning for the combine option, if accepted:
...
Row #4 (`.lsa/features/2026-05-22-lsa-what-why-preamble/`) is moving from "sub-bullet under Rule 5" to "new Rule 6" per its audit-driven decision. Both rows can't both be Rule 6.
