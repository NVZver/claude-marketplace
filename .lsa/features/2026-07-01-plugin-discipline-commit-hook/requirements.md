...
| F3 | **Plugin discovery.** The system shall treat as a plugin exactly those top-level directories that contain `.claude-plugin/plugin.json`, so repo-internal infra (`scripts/`, `.lsa/`, `.claude/`, `tests/`, root docs) is exempt by construction. Within a plugin, staged files under `<plugin>/tests/**` or `<plugin>/VERIFICATION.md` shall not trigger the F4/F5 discipline (test artifacts are outside the behavior surface, mirroring `.lsa.yaml` `artifact_paths`; maintainer-authorized 2026-07-02 after a drill-suite false-positive block — drift absorbed per Level 2.5). The F6 trace check remains unconditional. | Must |
