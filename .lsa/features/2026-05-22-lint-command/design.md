...
4. **Steps:**
   ...
   - Step 2: Enumerate scope. `full-scan` → `find . -name '*.md' -not -path './node_modules/*' -not -path './.git/*' | grep -v CHANGELOG | grep -v '.lsa/archive/' | grep -v '.lsa/plans/'`. `changes-only` → `git diff --name-only main` filtered to the same patterns.
