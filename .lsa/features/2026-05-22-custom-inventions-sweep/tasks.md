...
| T3 | Defer OQ1 (file-load trace) to next dogfood log | follow-up | #2, #5 | none (research entry) | next dogfood log row in `.lsa/roadmap.md:23` |
| T4 | Surface OQ2 (verb-headline vocabulary) in row #4's design.md | follow-up | #4 | none (cross-feature note) | row #4 backlog item gets specified (`.lsa/roadmap.md:36`) |
...
- `.lsa-sync-state.json` baseline-SHA file deleted. The per-module last-sync SHA + ISO timestamp the file held is recoverable from `git log -1 --format=%H -- <module-spec-path>`; switched `lsa-reconcile` Step 1, `lsa-sync` Step 6, and `lsa/hooks/session-start-drift-check.sh` to derive the SHA from git history instead. Per the custom-inventions sweep at `.lsa/features/2026-05-22-custom-inventions-sweep/design.md` inventory row #1.
```

**Verification:**
- `grep -rn "\.lsa-sync-state" lsa/ .lsa/` returns zero results outside archive files.
...
**Action:** Add a one-line row to `.lsa/research-backlog.md` (or create the file if absent per `lsa/ARCHITECTURE.md:54`) noting the open question and the resolution path (next dogfood log per `.lsa/roadmap.md:23`).

**Files touched:** `.lsa/research-backlog.md` (one row added).
...
**Action:** File OQ2 in `.lsa/features/2026-05-22-lsa-what-why-preamble/design.md` Open Questions section (line 174) so the verb-headline-vocabulary-keep-or-remove decision is made in the row-#4 spec context. The row-#4 feature spec already exists at `.lsa/features/2026-05-22-lsa-what-why-preamble/`.

**Files touched:** `.lsa/features/2026-05-22-lsa-what-why-preamble/design.md` (one entry added to §"Open Questions").
...
**Action:** On merge, edit `.lsa/roadmap.md` row *"Sweep custom inventions; remove the unjustified"* — move from Feature Backlog to Recently merged.

**Files touched:** `.lsa/roadmap.md` (one row moved).
...
5. `lsa-sync` extracts the delta into `lsa/`'s module spec at `.lsa/modules/lsa/spec.md` (the §"Decisions" or equivalent section gains a row noting the two removals + the three deferred OQs).
