...
| F6 | The agent shall sequence the resulting removal PRs in `tasks.md` from lowest to highest blast radius, so each removal lands as one independent PR per the trace-tag-removal exemplar (`.lsa/roadmap.md:44`, commit `c226623`). | Must |
...
| ID | Requirement |
|----|-------------|
| NF1 | The inventory shall surface at least 8 candidates — the 3 named in `.lsa/roadmap.md:139-141` plus a minimum of 5 newly surfaced by the scan — to satisfy the parent task's framing (*"the sweep itself will surface more"* — `.lsa/roadmap.md:138`). |
...
| NF3 | The rubric shall name exactly five adopted 3rd-party standards (EARS, SemVer, Keep a Changelog, CommonMark, Claude Code plugin schema) plus Claude Code substrate — no more, no less — so the rubric is verifiable. The five are enumerated in the parent task framing and at `.lsa/roadmap.md:138`. |
| NF4 | The sweep shall not edit any archive file under `.lsa/archive/**/` or `.lsa/plans/2026-05-20-*` — those are frozen historical records per the trace-tag-removal precedent (`lsa/CHANGELOG.md:7-14`). |
...
**Inputs.**
- `.lsa/roadmap.md:134-141` — the user's verbatim Problem / Example / Expected Output text.
- `.lsa/roadmap.md:44` and `lsa/CHANGELOG.md:7-14` — the trace-tag-removal exemplar (commit `c226623`).
- `lsa/knowledge/conventions.md` — current LSA-internal vocabulary surface.
- `.lsa/VISION.md` §2 (especially principles 6, 7, 9) — the constitutional anchor.
- `.lsa/VISION.md:204` — EARS five-pattern definition.
- The full skill / agent / knowledge file inventory under `lsa/`, `core/`, `helper/`.
- The Claude Code plugin schema (referenced by `*.claude-plugin/plugin.json` in all three plugins) and the [plugin-dependencies docs](https://code.claude.com/docs/en/plugin-dependencies) (cited at `.lsa/roadmap.md:25-26`).
...
- **Archive files are frozen.** Per `lsa/CHANGELOG.md:7-14`, archive files (`.lsa/archive/**/`, `.lsa/plans/2026-05-20-*`) are intentionally untouched — historical references resolve via the current CHANGELOG entries.
...
- Refactoring Helper from command-router to assistant (row #1, `.lsa/roadmap.md:33`).
- Helper fast-path for onboarding questions (row #2, `.lsa/roadmap.md:34`).
- Audit + tighten `AskUserQuestion` call sites (row #3, `.lsa/roadmap.md:35`) — different decision rubric (when to ask, not what to keep).
- LSA verb-headline what-and-why preamble (row #4, `.lsa/roadmap.md:36`) — interacts with the verb-headline-vocabulary candidate in this sweep; see `design.md` §"PR sequencing → row #4 interaction".
- Show actual changes inline (row #5, `.lsa/roadmap.md:37`) — separate behavioral discipline.
- Reconcile-classification automation (`.lsa/roadmap.md:32`) — Class (a) / (b) is a deliberate LSA-internal taxonomy; in scope only as an inventory entry, not for removal in this feature.
- Lints / harness checks that auto-detect new inventions (deferred to the Self-eval harness row in `.lsa/roadmap.md:28`).
...
- **AC7** — *Ubiquitous*. No file under `.lsa/archive/**/` or `.lsa/plans/2026-05-20-*` is modified by any PR generated from this feature. *Observable*: `git diff` of every PR touches zero archive files.
