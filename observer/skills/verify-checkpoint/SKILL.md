...
On each `/loop` cycle, detect whether an implementer has signalled a finished increment and, if so, grade that one increment does·only against its target F-requirement — emitting an independent CLEAR verdict that auto-clears the boundary, or a BLOCK verdict naming the failing check and surfaced to the human before the next task — while writing nothing to the artifacts it grades.

## Input

- The checkpoint signal note (fields above), if one is present this cycle. Re-read every cycle; it is the source of truth across stateless wakes.
...
1. **Start the verification loop** (F2). Start the substrate's self-paced `/loop` (omit the interval). Do not build a scheduler, timer, or poll ([`../../../.lsa/VISION.md`](../../../.lsa/VISION.md) principle 9). Observable result: a self-paced `/loop` is running with this Actor as its per-cycle prompt.

2. **Per cycle — detect a checkpoint signal** (F3, F4). On each wake, look for the checkpoint-signal note and read its `status`. If no note is present, or `status` is not the pause marker, this cycle has **no signal**: produce no verdict and no user-facing output — silence means NO user-facing text (no marker, token, placeholder, status line, or parenthetical such as `<no-signal>` or `(idle)`), and no narration of the decision to stay silent. A no-signal cycle produces zero output and ends here (mirrors `observe`'s silence discipline, [`../observe/SKILL.md:37`](../observe/SKILL.md) step 6d). Observable result: on a signal-less cycle, zero output; otherwise, continue to Step 3.
