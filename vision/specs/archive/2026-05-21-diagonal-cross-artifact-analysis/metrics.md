...
- ACs declared: 6 (`vision/specs/features/diagonal-cross-artifact-analysis/requirements.md` § Acceptance Criteria — AC1 through AC6, AC6 added retroactively at commit 16b2a6f to close lsa-verify W1)
...
- **Score:** 57/60 ≈ 0.95
- **Note:** the 3-claim gap is approximate — a rigorous count would re-grep each file and adjudicate per-sentence; deferred per the test-skill `vision/specs/standards/testing.md` *"Statistical eval explicitly deferred"*. Pass/fail count is honest.
...
The strict metric above counts only `artifact_paths`-resident files (per `.lsa.yaml:9-26`). The full feature-branch diff included 13 files; 10 fall outside `artifact_paths` (feature spec dir × 7, `vision/specs/main.spec.md`, `vision/specs/modules/lsa/spec.md`, root `README.md`). Of these 10:
