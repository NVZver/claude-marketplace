...
| 4 | Error — PR touches an archive file | Agent runs `git diff --name-only` → finds a path under `vision/specs/archive/**/` or `vision/plans/2026-05-20-*` → AC7 fails → revert the archive-file edit; archives are frozen per `lsa/CHANGELOG.md:7-14` |
...
| 1a | EARS-pattern | ✓ | requirements.md §"Acceptance Criteria" maps each AC to one of Ubiquitous / Event / Unwanted per `vision/VISION.md:204` |
| 1b | Journey-shape | ✓ | Every AC names a user-observable behavior (reader opens file, reader sees row, reader counts rows, agent diffs PR) at the user/system boundary — not unit-test scope. Per `vision/VISION.md` §2 sub-principle 2a |
