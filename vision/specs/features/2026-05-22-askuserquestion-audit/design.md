...
1. **PR-A (`core`)** — add Rule 5 sub-rule "Genuine-fork test" to `core/skills/output/SKILL.md`. Citing Principle 9 (`vision/VISION.md:66`) by link. Update `core/CLAUDE.md` operational checkpoint #1 to make the orthogonality explicit. Bump `core/plugin.json` minor version; entry in `core/CHANGELOG.md`. **Ships first** — Helper and LSA PRs cite this rule.
...
| H5f | `helper/agents/helper.md:52` | Constraint — substrate-native decisions use `AskUserQuestion`, never `[a]/[b]/[c]` | `meta-reference` | Restates Principle 9 binding (`vision/VISION.md:66`). |
...
| H5j | `helper/commands/help.md:29` | Constraint — never render `[a] / [b] / [c]`, always `AskUserQuestion`. Cites `vision/VISION.md:66` Principle 9. | `meta-reference` | Restates Principle 9. |
...
| L3 | `lsa/skills/lsa-specify/SKILL.md:40` | Step opening — *"answers captured; human approval logged"* | `keep` | Genuine fork — spec content the user must own (per `vision/VISION.md:15` "ownership over automation"). |
...
| L8 | `lsa/skills/lsa-plan/SKILL.md:120` | Plan approval — before implementation starts | `keep` | Destructive; human owns intent per `vision/VISION.md:64` Principle 7. |
...
The audit confirms the *posture* problem the user reported lives primarily in **Helper's mandatory closing picker** (H3 + `output-discipline.md:20`) rather than in LSA's substantive gates. This validates the "partially orthogonal to #1" framing in `vision/specs/roadmap.md:116`.
...
2. **Two named designs in scope and neither overrides the other** — the agent has identified ≥2 reasonable continuations from in-scope sources (`vision/VISION.md:63` Principle 6: in-repo config → in-repo docs → code → external → ask) and no source ranks one above the other.
...
Backlog #1 (Helper command-router refactor, `vision/specs/roadmap.md:104-108`) rewrites Helper's **default flow** to lead with a cited answer, demoting dispatch to a side effect. The overlap with this row is at H3 and `output-discipline.md:20` (the mandatory closing picker).
...
- **OQ6.** Should a separate VISION-citation-refresh PR ride along on PR-A? Beyond this feature dir, the repo has stale `vision/VISION.md` line citations from prior reorders (e.g., `:55`, `:57`, `:63`, `:201` patterns may point at the wrong principle elsewhere). A repo-wide grep + refresh sweep is one extra commit in PR-A; alternatively, defer to `lsa-reconcile`. Recommend: piggyback on PR-A — citations are part of "trustworthy output" (`vision/VISION.md:15`).
