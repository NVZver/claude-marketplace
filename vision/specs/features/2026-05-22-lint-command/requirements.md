...
| F2 | Supports two modes: `full-scan` (default — every `.md` under `core/`, `lsa/`, `helper/`, `vision/`, root) and `changes-only` (diff vs `main`). Selected via `--mode=` flag. | Must |
...
- Banned-hedge-word lint — deferred to the Self-eval harness row in `vision/specs/roadmap.md`.
