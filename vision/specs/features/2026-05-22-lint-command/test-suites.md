...
| # | Path | Actions |
|---|------|---------|
| 1 | Happy | maintainer invokes `/lint` (no args) → command reads CONTRIBUTING.md + enumerates scope (`core/`, `lsa/`, `helper/`, `vision/`, root `*.md`) → produces severity-grouped Markdown report (sorted High → Medium → Low, then `file:line` asc) → maintainer triages findings. Report length scales with finding count; no cap. |
