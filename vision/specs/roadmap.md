...
| `core/output` discipline enforcement (`AskUserQuestion` + output length) | Should | backlog | Per user observations 2026-05-21 — two `core/output` rules routinely skipped in practice: (a) Principle 9 (`vision/VISION.md:63` *"In Claude Code: `AskUserQuestion` for decisions"*) — agents render text `[a]/[b]/[c]` blocks where the native picker was available; (b) "Minimal" + "below-the-fold" — agents push 60+ line responses with tables + worked-example + decision block in one turn, forcing the user to scroll-and-skim. Three fix-points: (1) elevate both rules to always-on bullets in `core/CLAUDE.md` (currently embedded inside `core/ground-rules` Rule 0 and `core/output` body respectively); (2) tighten `core/output` body to mandate "1–1.5 screen budget; split into turns; pull don't push" as concrete shape; (3) add lints (text-decision-block-where-picker-available, response-line-budget) to the deferred Self-eval harness row below. Fix-points #1 + #2 can land standalone; #3 rolls into the v0.3.0 harness. |
| Rename `lsa-specify` "Gate N" → "User Verification: <name>" | Should | backlog | Per user observation 2026-05-21 — "Gate 1, 2, 3" is opaque; you have to know the contents to read it. Proposed: "User Verification: Requirements + Contract Trigger" / "User Verification: Test Suites + Contract + Design" / "User Verification: Final Integration". Surfaced while Helper-persona was explaining LSA — evidence that Helper-mode is a useful UX-design check on other skills. Scope: 103 references across 14 files (`grep "Gate [123]"`) — `lsa/skills/lsa-specify/SKILL.md`, `lsa/ARCHITECTURE.md`, `lsa/CHANGELOG.md`, `lsa/README.md`, `vision/VISION.md`, `vision/specs/modules/lsa/spec.md`, plus 7 diagonal-feature archive files. Land as `lsa` minor bump with CHANGELOG entry; archive files get a one-line back-link note rather than rewrite. |
| Rename `T1` / `T2` / `T3` → `Flow: Quick` / `Flow: Standard` / `Flow: Extended` | Should | backlog | Per user observation 2026-05-21 — same opaque-position pattern as "Gate N"; T1/T2/T3 forces re-grounding every turn (Helper-mode found it). Current secondary labels (Quick / Standard / Full per `core/CLAUDE.md` §"Tier outcomes") become primary; "Full" → "Extended". Framing shifts from "ceremony tier" (hierarchy) to "flow type" (process). Scope: 151 references across 34 files (`grep -E "\bT[123]\b\|Tier [123]"`). Highest-traffic files: `core/skills/tier-selector/SKILL.md`, `core/CLAUDE.md`, `vision/VISION.md` §4, every `lsa/skills/*/SKILL.md`, both READMEs, both CHANGELOGs, every archive file. Also rename `core/tier-selector` skill itself — candidate: `core/flow-selector`. Touches both `core` and `lsa` plugin major-ish bumps. |
| LSA gate prompts must be concrete (no IDs, no jargon, must-decide only) | **Must** | backlog | Per user observation 2026-05-21 — dominant reason `lsa-specify` "takes forever". Quote: *"I have no IDEA what it means…wording is too…i don't know, it just means nothing to me…I want concrete questions to make decisions with clear problem to solve. I do not give a fuck about minor things."* Three fix-points: (1) **No opaque IDs in user-facing prompts** — `F1` / `AC2` / `OQ5` stay in spec files for traceability, but every `AskUserQuestion` resolves them to real-world subject phrasing ("Add password reset endpoint?" not "Approve F3?"). (2) **Strip project jargon from prompts** — `contract-trigger` / `Hard Confirm` / `diagonal coverage` / `epic decomposition` etc. belong in skill bodies, not user-facing pickers. (3) **Bundle nice-to-decide; surface only must-decide** — distinguish questions that meaningfully change outcome from consistency checks. Touches `lsa/skills/lsa-specify/SKILL.md` Step 4 + Step 5 (prompt drafting), `lsa-plan` review gate, `lsa-init` brownfield gate, possibly `core/output` body for a "prompt voice" rule. Promote `<dimension>: <name>` naming + "prompt voice" rule as twin LSA UX standards. Highest-priority discipline gap surfaced this session. |
