...
| Module | Spec | Status |
|---|---|---|
| `core` | [`vision/specs/modules/core/spec.md`](./modules/core/spec.md) | active — v0.5.3 |
| `lsa` | [`vision/specs/modules/lsa/spec.md`](./modules/lsa/spec.md) | active — v0.6.3 | <!-- added: 2026-05-21-ears-journey-shape-ac 2026-05-21; bumped: fix/discipline-ground + fix/naming-clarity 2026-05-22; bumped: feature/2026-05-21-maintenance-cleanup 2026-05-22 -->
| `maintenance` | [`vision/specs/modules/maintenance/spec.md`](./modules/maintenance/spec.md) | active — v0.1.0 | <!-- added: feature/2026-05-21-maintenance-cleanup 2026-05-22 -->
...
- **NFR2 — Spec-grounding.** Every artifact change traces to a spec requirement. Direct edits are absorbed (Level 2.5) rather than blocked — via `lsa-reconcile`. Source: `vision/VISION.md:135`.
...
- **NFR6 — Level 2.5 reconcile.** Direct artifact edits are detected, surfaced via the SessionStart drift hook, and absorbed into the spec via `lsa-reconcile` — never blocked or reverted. Source: `vision/VISION.md:144`.
...
| `.lsa-sync-state.json` | LSA's per-module last-sync state. Written by `lsa-sync`. |
| `.gitignore`, `.editorconfig`, etc. | Repo plumbing. |

Changes here are tracked by git but do not trigger `lsa-verify`. If marketplace.json or .lsa.yaml introduce *behavioral* changes (e.g., adding a new plugin, changing `mode`), the change appears in the main.spec.md module index and is reviewed there.
