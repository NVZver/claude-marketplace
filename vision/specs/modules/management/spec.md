...
**Plugin manifest:** [`management/.claude-plugin/plugin.json`](../../../../management/.claude-plugin/plugin.json) (v0.2.0)
...
- **Versioning.** `management` evolves with its own SemVer + CHANGELOG (`vision/VISION.md` §1 *"Distribution + versioning"*). Currently v0.2.0.
...
- **Ownership over automation.** Both agents facilitate — they do not decide scope or sequencing. Per `vision/VISION.md:15`.
...
- **Roadmap is the single entry point for the project-manager.** The agent reads `vision/specs/roadmap.md` as its primary data source. All roadmap modifications are proposed as inline diffs; no silent writes.
