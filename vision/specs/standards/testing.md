...
Applies to `core/ground-rules`, `core/actor-template`, `core/flow-selector` (renamed from `core/tier-selector` in `core` v0.5.2), `discover`, and `reconcile`. Per `vision/specs/2026-05-20-lsa-v0.2.0-design.md` §13 *"Falsifiable threshold"*.
...
The three metrics — accuracy to task, proven facts with sources, only-required-changes — are tracked as pass/fail counts per archived Extended-flow feature (was `T3`) in `${specs_root}/archive/<feature>/metrics.md`, written by `verify` on clean PASS. Per `vision/specs/2026-05-20-lsa-v0.2.0-design.md` §9.
