...
**Status:** `[todo — restructure pending review]` — audit-C (2026-05-20) flagged DRY/KISS/SRP defects in PR 1's structure: output discipline conflated with content rules inside `ground-rules`; `core/CLAUDE.md` restated the 8 rules instead of citing them; probes restated rule behaviors per-rule. Restructure: extract output discipline to its own skill (`core/skills/output`) with 4 golden rules; ground-rules stays content-only (6 rules); every component picks its own output format within the golden rules; substrate-native principle goes to Vision §2. PR 1 commits 3dc1828 + 53d7c58 will be discarded by `git reset --hard 01126d1` once this restructure is approved.
**Date:** 2026-05-20 (audit-A: Rule 1 reference discipline; audit-B: post-refactor remap; audit-C: DRY/KISS/SRP restructure)
...
**Audit-C resolutions (2026-05-20, post-PR1-self-review):**
